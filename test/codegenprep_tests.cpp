#include "testutils.h"
#include "gmock/gmock.h"
#include "gtest/gtest.h"
#include <filesystem>
#include <iostream>
#include <string>

extern "C"
{
#include "test_interface.h"
#include "to_string.h"
}

class CodeGenPrepTest : public testing::Test
{
  public:
    char *root_string = nullptr;
    char *symbols_string = nullptr;
    node_st *root = nullptr;
    std::string input_filepath;
    std::string err_output;
    std::string std_output;

  protected:
    CodeGenPrepTest()
    {
    }

    void SetUpNoExecute(std::string filepath)
    {
        input_filepath =
            std::filesystem::absolute(std::string(PROJECT_DIRECTORY) + "/test/data/" + filepath);
        ASSERT_TRUE(std::filesystem::exists(input_filepath))
            << "File does not exist at path '" << input_filepath << "'";
    }

    void SetUp(std::string filepath)
    {
        SetUpNoExecute(filepath);
        testing::internal::CaptureStdout();
        testing::internal::CaptureStderr();
        root = run_code_gen_preparation(input_filepath.c_str());
        EXPECT_NE(nullptr, root) << "Could not parse ast in file: '" << input_filepath << "'";
        err_output = testing::internal::GetCapturedStderr();
        std_output = testing::internal::GetCapturedStdout();
        ASSERT_THAT(err_output,
                    testing::Not(testing::HasSubstr("error: Inconsistent node found in AST")));
        root_string = node_to_string(root);
        symbols_string = symbols_to_string(root);
    }

    void TearDown() override
    {
        if (root_string != nullptr)
        {

            if (HasFailure())
            {
                std::cerr
                    << "========================================================================\n"
                    << "                        Node Representation\n"
                    << "========================================================================\n"
                    << root_string << std::endl;
            }

            free(root_string);
        }

        if (symbols_string != nullptr)
        {
            if (HasFailure())
            {
                std::cerr
                    << "========================================================================\n"
                    << "                        Symbol Tables of AST\n"
                    << "========================================================================\n"
                    << symbols_string << std::endl;
            }

            free(symbols_string);
        }

        if (root != nullptr)
        {
            cleanup_nodes(root);
            root = nullptr;
        }
    }
};

TEST_F(CodeGenPrepTest, GlobalArrayAssignments)
{
    SetUp("milestone_6/array_init/main.cvc");
    ASSERT_NE(nullptr, root);

    const char *expected = "Program\n"
                           "┢─ Declarations\n"
                           "┃  └─ GlobalDef -- has_export:'0'\n"
                           "┃     └─ VarDec -- type:'int'\n"
                           "┃        ├─ ArrayExpr\n"
                           "┃        │  ┢─ Exprs\n"
                           "┃        │  ┃  └─ Int -- val:'5'\n"
                           "┃        │  ┡─ NULL\n"
                           "┃        │  └─ Var -- name:'arr1'\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ GlobalDef -- has_export:'0'\n"
                           "┃     └─ VarDec -- type:'int'\n"
                           "┃        ├─ ArrayExpr\n"
                           "┃        │  ┢─ Exprs\n"
                           "┃        │  ┃  └─ Int -- val:'2'\n"
                           "┃        │  ┣─ Exprs\n"
                           "┃        │  ┃  └─ Int -- val:'3'\n"
                           "┃        │  ┡─ NULL\n"
                           "┃        │  └─ Var -- name:'arr2'\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ GlobalDef -- has_export:'0'\n"
                           "┃     └─ VarDec -- type:'int'\n"
                           "┃        ├─ ArrayExpr\n"
                           "┃        │  ┢─ Exprs\n"
                           "┃        │  ┃  └─ Int -- val:'2'\n"
                           "┃        │  ┣─ Exprs\n"
                           "┃        │  ┃  └─ Int -- val:'3'\n"
                           "┃        │  ┣─ Exprs\n"
                           "┃        │  ┃  └─ Int -- val:'4'\n"
                           "┃        │  ┡─ NULL\n"
                           "┃        │  └─ Var -- name:'arr3'\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ GlobalDef -- has_export:'0'\n"
                           "┃     └─ VarDec -- type:'int'\n"
                           "┃        ├─ Var -- name:'a'\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ FunDef -- has_export:'1'\n"
                           "┃     ├─ FunHeader -- type:'void'\n"
                           "┃     │  ├─ Var -- name:'__init'\n"
                           "┃     │  └─ NULL\n"
                           "┃     └─ FunBody\n"
                           "┃        ┢─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_dims0'\n"
                           "┃        ┃     └─ NULL\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_dims1'\n"
                           "┃        ┃     └─ NULL\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_dims2'\n"
                           "┃        ┃     └─ NULL\n"
                           "┃        ┡─ NULL\n"
                           "┃        ├─ NULL\n"
                           "┃        ┢─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'@loop_dims0'\n"
                           "┃        ┃     └─ Int -- val:'5'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'arr1'\n"
                           "┃        ┃     └─ ProcCall\n"
                           "┃        ┃        ├─ Var -- name:'@alloc'\n"
                           "┃        ┃        ┢─ Exprs\n"
                           "┃        ┃        ┃  └─ Var -- name:'@loop_dims0'\n"
                           "┃        ┃        ┗─ NULL\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'@loop_dims1'\n"
                           "┃        ┃     └─ Binop -- op:'*'\n"
                           "┃        ┃        ├─ Int -- val:'2'\n"
                           "┃        ┃        └─ Int -- val:'3'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'arr2'\n"
                           "┃        ┃     └─ ProcCall\n"
                           "┃        ┃        ├─ Var -- name:'@alloc'\n"
                           "┃        ┃        ┢─ Exprs\n"
                           "┃        ┃        ┃  └─ Var -- name:'@loop_dims1'\n"
                           "┃        ┃        ┗─ NULL\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'@loop_dims2'\n"
                           "┃        ┃     └─ Binop -- op:'*'\n"
                           "┃        ┃        ├─ Binop -- op:'*'\n"
                           "┃        ┃        │  ├─ Int -- val:'2'\n"
                           "┃        ┃        │  └─ Int -- val:'3'\n"
                           "┃        ┃        └─ Int -- val:'4'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'arr3'\n"
                           "┃        ┃     └─ ProcCall\n"
                           "┃        ┃        ├─ Var -- name:'@alloc'\n"
                           "┃        ┃        ┢─ Exprs\n"
                           "┃        ┃        ┃  └─ Var -- name:'@loop_dims2'\n"
                           "┃        ┃        ┗─ NULL\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Int -- val:'0'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr1'\n"
                           "┃        ┃     └─ Int -- val:'1'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Int -- val:'1'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr1'\n"
                           "┃        ┃     └─ Int -- val:'2'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr1'\n"
                           "┃        ┃     └─ Int -- val:'3'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Int -- val:'3'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr1'\n"
                           "┃        ┃     └─ Int -- val:'4'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Int -- val:'4'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr1'\n"
                           "┃        ┃     └─ Int -- val:'5'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  ├─ Int -- val:'0'\n"
                           "┃        ┃     │  ┃     │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     └─ Int -- val:'0'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr2'\n"
                           "┃        ┃     └─ Int -- val:'1'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  ├─ Int -- val:'0'\n"
                           "┃        ┃     │  ┃     │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     └─ Int -- val:'1'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr2'\n"
                           "┃        ┃     └─ Int -- val:'2'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  ├─ Int -- val:'0'\n"
                           "┃        ┃     │  ┃     │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr2'\n"
                           "┃        ┃     └─ Int -- val:'3'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  ├─ Int -- val:'1'\n"
                           "┃        ┃     │  ┃     │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     └─ Int -- val:'0'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr2'\n"
                           "┃        ┃     └─ Int -- val:'4'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  ├─ Int -- val:'1'\n"
                           "┃        ┃     │  ┃     │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     └─ Int -- val:'1'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr2'\n"
                           "┃        ┃     └─ Int -- val:'5'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  ├─ Int -- val:'1'\n"
                           "┃        ┃     │  ┃     │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr2'\n"
                           "┃        ┃     └─ Int -- val:'6'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  ├─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     │  │  ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  │  │  ├─ Int -- val:'0'\n"
                           "┃        ┃     │  ┃     │  │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  │  └─ Int -- val:'0'\n"
                           "┃        ┃     │  ┃     │  └─ Int -- val:'3'\n"
                           "┃        ┃     │  ┃     └─ Int -- val:'0'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr3'\n"
                           "┃        ┃     └─ Int -- val:'1'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  ├─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     │  │  ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  │  │  ├─ Int -- val:'0'\n"
                           "┃        ┃     │  ┃     │  │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  │  └─ Int -- val:'0'\n"
                           "┃        ┃     │  ┃     │  └─ Int -- val:'3'\n"
                           "┃        ┃     │  ┃     └─ Int -- val:'1'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr3'\n"
                           "┃        ┃     └─ Int -- val:'2'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  ├─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     │  │  ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  │  │  ├─ Int -- val:'0'\n"
                           "┃        ┃     │  ┃     │  │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  │  └─ Int -- val:'0'\n"
                           "┃        ┃     │  ┃     │  └─ Int -- val:'3'\n"
                           "┃        ┃     │  ┃     └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr3'\n"
                           "┃        ┃     └─ Int -- val:'3'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  ├─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     │  │  ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  │  │  ├─ Int -- val:'0'\n"
                           "┃        ┃     │  ┃     │  │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  │  └─ Int -- val:'0'\n"
                           "┃        ┃     │  ┃     │  └─ Int -- val:'3'\n"
                           "┃        ┃     │  ┃     └─ Int -- val:'3'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr3'\n"
                           "┃        ┃     └─ Int -- val:'4'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  ├─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     │  │  ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  │  │  ├─ Int -- val:'0'\n"
                           "┃        ┃     │  ┃     │  │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  │  └─ Int -- val:'1'\n"
                           "┃        ┃     │  ┃     │  └─ Int -- val:'3'\n"
                           "┃        ┃     │  ┃     └─ Int -- val:'0'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr3'\n"
                           "┃        ┃     └─ Int -- val:'5'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  ├─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     │  │  ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  │  │  ├─ Int -- val:'0'\n"
                           "┃        ┃     │  ┃     │  │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  │  └─ Int -- val:'1'\n"
                           "┃        ┃     │  ┃     │  └─ Int -- val:'3'\n"
                           "┃        ┃     │  ┃     └─ Int -- val:'1'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr3'\n"
                           "┃        ┃     └─ Int -- val:'6'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  ├─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     │  │  ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  │  │  ├─ Int -- val:'0'\n"
                           "┃        ┃     │  ┃     │  │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  │  └─ Int -- val:'1'\n"
                           "┃        ┃     │  ┃     │  └─ Int -- val:'3'\n"
                           "┃        ┃     │  ┃     └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr3'\n"
                           "┃        ┃     └─ Int -- val:'7'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  ├─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     │  │  ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  │  │  ├─ Int -- val:'0'\n"
                           "┃        ┃     │  ┃     │  │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  │  └─ Int -- val:'1'\n"
                           "┃        ┃     │  ┃     │  └─ Int -- val:'3'\n"
                           "┃        ┃     │  ┃     └─ Int -- val:'3'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr3'\n"
                           "┃        ┃     └─ Int -- val:'8'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  ├─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     │  │  ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  │  │  ├─ Int -- val:'0'\n"
                           "┃        ┃     │  ┃     │  │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  └─ Int -- val:'3'\n"
                           "┃        ┃     │  ┃     └─ Int -- val:'0'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr3'\n"
                           "┃        ┃     └─ Int -- val:'9'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  ├─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     │  │  ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  │  │  ├─ Int -- val:'0'\n"
                           "┃        ┃     │  ┃     │  │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  └─ Int -- val:'3'\n"
                           "┃        ┃     │  ┃     └─ Int -- val:'1'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr3'\n"
                           "┃        ┃     └─ Int -- val:'10'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  ├─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     │  │  ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  │  │  ├─ Int -- val:'0'\n"
                           "┃        ┃     │  ┃     │  │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  └─ Int -- val:'3'\n"
                           "┃        ┃     │  ┃     └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr3'\n"
                           "┃        ┃     └─ Int -- val:'11'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  ├─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     │  │  ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  │  │  ├─ Int -- val:'0'\n"
                           "┃        ┃     │  ┃     │  │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  └─ Int -- val:'3'\n"
                           "┃        ┃     │  ┃     └─ Int -- val:'3'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr3'\n"
                           "┃        ┃     └─ Int -- val:'12'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  ├─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     │  │  ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  │  │  ├─ Int -- val:'1'\n"
                           "┃        ┃     │  ┃     │  │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  │  └─ Int -- val:'0'\n"
                           "┃        ┃     │  ┃     │  └─ Int -- val:'3'\n"
                           "┃        ┃     │  ┃     └─ Int -- val:'0'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr3'\n"
                           "┃        ┃     └─ Int -- val:'13'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  ├─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     │  │  ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  │  │  ├─ Int -- val:'1'\n"
                           "┃        ┃     │  ┃     │  │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  │  └─ Int -- val:'0'\n"
                           "┃        ┃     │  ┃     │  └─ Int -- val:'3'\n"
                           "┃        ┃     │  ┃     └─ Int -- val:'1'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr3'\n"
                           "┃        ┃     └─ Int -- val:'14'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  ├─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     │  │  ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  │  │  ├─ Int -- val:'1'\n"
                           "┃        ┃     │  ┃     │  │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  │  └─ Int -- val:'0'\n"
                           "┃        ┃     │  ┃     │  └─ Int -- val:'3'\n"
                           "┃        ┃     │  ┃     └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr3'\n"
                           "┃        ┃     └─ Int -- val:'15'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  ├─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     │  │  ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  │  │  ├─ Int -- val:'1'\n"
                           "┃        ┃     │  ┃     │  │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  │  └─ Int -- val:'0'\n"
                           "┃        ┃     │  ┃     │  └─ Int -- val:'3'\n"
                           "┃        ┃     │  ┃     └─ Int -- val:'3'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr3'\n"
                           "┃        ┃     └─ Int -- val:'16'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  ├─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     │  │  ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  │  │  ├─ Int -- val:'1'\n"
                           "┃        ┃     │  ┃     │  │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  │  └─ Int -- val:'1'\n"
                           "┃        ┃     │  ┃     │  └─ Int -- val:'3'\n"
                           "┃        ┃     │  ┃     └─ Int -- val:'0'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr3'\n"
                           "┃        ┃     └─ Int -- val:'17'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  ├─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     │  │  ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  │  │  ├─ Int -- val:'1'\n"
                           "┃        ┃     │  ┃     │  │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  │  └─ Int -- val:'1'\n"
                           "┃        ┃     │  ┃     │  └─ Int -- val:'3'\n"
                           "┃        ┃     │  ┃     └─ Int -- val:'1'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr3'\n"
                           "┃        ┃     └─ Int -- val:'18'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  ├─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     │  │  ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  │  │  ├─ Int -- val:'1'\n"
                           "┃        ┃     │  ┃     │  │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  │  └─ Int -- val:'1'\n"
                           "┃        ┃     │  ┃     │  └─ Int -- val:'3'\n"
                           "┃        ┃     │  ┃     └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr3'\n"
                           "┃        ┃     └─ Int -- val:'19'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  ├─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     │  │  ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  │  │  ├─ Int -- val:'1'\n"
                           "┃        ┃     │  ┃     │  │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  │  └─ Int -- val:'1'\n"
                           "┃        ┃     │  ┃     │  └─ Int -- val:'3'\n"
                           "┃        ┃     │  ┃     └─ Int -- val:'3'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr3'\n"
                           "┃        ┃     └─ Int -- val:'20'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  ├─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     │  │  ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  │  │  ├─ Int -- val:'1'\n"
                           "┃        ┃     │  ┃     │  │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  └─ Int -- val:'3'\n"
                           "┃        ┃     │  ┃     └─ Int -- val:'0'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr3'\n"
                           "┃        ┃     └─ Int -- val:'21'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  ├─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     │  │  ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  │  │  ├─ Int -- val:'1'\n"
                           "┃        ┃     │  ┃     │  │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  └─ Int -- val:'3'\n"
                           "┃        ┃     │  ┃     └─ Int -- val:'1'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr3'\n"
                           "┃        ┃     └─ Int -- val:'22'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  ├─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     │  │  ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  │  │  ├─ Int -- val:'1'\n"
                           "┃        ┃     │  ┃     │  │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  └─ Int -- val:'3'\n"
                           "┃        ┃     │  ┃     └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr3'\n"
                           "┃        ┃     └─ Int -- val:'23'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ArrayAssign\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  ├─ Binop -- op:'+'\n"
                           "┃        ┃     │  ┃     │  │  ├─ Binop -- op:'*'\n"
                           "┃        ┃     │  ┃     │  │  │  ├─ Int -- val:'1'\n"
                           "┃        ┃     │  ┃     │  │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  │  └─ Int -- val:'2'\n"
                           "┃        ┃     │  ┃     │  └─ Int -- val:'3'\n"
                           "┃        ┃     │  ┃     └─ Int -- val:'3'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr3'\n"
                           "┃        ┃     └─ Int -- val:'24'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'a'\n"
                           "┃        ┃     └─ Int -- val:'5'\n"
                           "┃        ┗─ NULL\n"
                           "┗─ NULL\n";

    ASSERT_MLSTREQ(expected, root_string);

    expected = "┌─ 0: Program\n"
               "├─ __init: FunHeader -- type:'void' -- Params: (null)\n"
               "├─ arr1: VarDec -- type:'int'\n"
               "├─ arr2: VarDec -- type:'int'\n"
               "├─ arr3: VarDec -- type:'int'\n"
               "└────────────────────\n"
               "\n"
               "┌─ 1: FunDef '__init' -- parent: '0: Program'\n"
               "├─ @loop_dims0: VarDec -- type:'int'\n"
               "├─ @loop_dims1: VarDec -- type:'int'\n"
               "├─ @loop_dims2: VarDec -- type:'int'\n"
               "└────────────────────\n";

    ASSERT_MLSTREQ(expected, symbols_string);
}

TEST_F(CodeGenPrepTest, InitFun)
{
    SetUp("milestone_6/init_fun/main.cvc");
    ASSERT_NE(nullptr, root);

    const char *expected = "Program\n"
                           "┢─ Declarations\n"
                           "┃  └─ GlobalDef -- has_export:'0'\n"
                           "┃     └─ VarDec -- type:'int'\n"
                           "┃        ├─ Var -- name:'aa'\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ GlobalDef -- has_export:'0'\n"
                           "┃     └─ VarDec -- type:'int'\n"
                           "┃        ├─ Var -- name:'ab'\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ GlobalDef -- has_export:'0'\n"
                           "┃     └─ VarDec -- type:'float'\n"
                           "┃        ├─ Var -- name:'ba'\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ GlobalDef -- has_export:'0'\n"
                           "┃     └─ VarDec -- type:'float'\n"
                           "┃        ├─ Var -- name:'bb'\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ GlobalDef -- has_export:'0'\n"
                           "┃     └─ VarDec -- type:'bool'\n"
                           "┃        ├─ Var -- name:'ca'\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ GlobalDef -- has_export:'0'\n"
                           "┃     └─ VarDec -- type:'bool'\n"
                           "┃        ├─ Var -- name:'cb'\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ GlobalDef -- has_export:'0'\n"
                           "┃     └─ VarDec -- type:'int'\n"
                           "┃        ├─ ArrayExpr\n"
                           "┃        │  ┢─ Exprs\n"
                           "┃        │  ┃  └─ Int -- val:'2'\n"
                           "┃        │  ┡─ NULL\n"
                           "┃        │  └─ Var -- name:'da'\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ GlobalDef -- has_export:'0'\n"
                           "┃     └─ VarDec -- type:'int'\n"
                           "┃        ├─ ArrayExpr\n"
                           "┃        │  ┢─ Exprs\n"
                           "┃        │  ┃  └─ Int -- val:'3'\n"
                           "┃        │  ┡─ NULL\n"
                           "┃        │  └─ Var -- name:'db'\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ GlobalDef -- has_export:'0'\n"
                           "┃     └─ VarDec -- type:'float'\n"
                           "┃        ├─ ArrayExpr\n"
                           "┃        │  ┢─ Exprs\n"
                           "┃        │  ┃  └─ Int -- val:'4'\n"
                           "┃        │  ┡─ NULL\n"
                           "┃        │  └─ Var -- name:'ea'\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ GlobalDef -- has_export:'0'\n"
                           "┃     └─ VarDec -- type:'float'\n"
                           "┃        ├─ ArrayExpr\n"
                           "┃        │  ┢─ Exprs\n"
                           "┃        │  ┃  └─ Int -- val:'6'\n"
                           "┃        │  ┡─ NULL\n"
                           "┃        │  └─ Var -- name:'eb'\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ GlobalDef -- has_export:'0'\n"
                           "┃     └─ VarDec -- type:'bool'\n"
                           "┃        ├─ ArrayExpr\n"
                           "┃        │  ┢─ Exprs\n"
                           "┃        │  ┃  └─ Int -- val:'7'\n"
                           "┃        │  ┡─ NULL\n"
                           "┃        │  └─ Var -- name:'fa'\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ GlobalDef -- has_export:'0'\n"
                           "┃     └─ VarDec -- type:'bool'\n"
                           "┃        ├─ ArrayExpr\n"
                           "┃        │  ┢─ Exprs\n"
                           "┃        │  ┃  └─ Int -- val:'9'\n"
                           "┃        │  ┡─ NULL\n"
                           "┃        │  └─ Var -- name:'fb'\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ FunDef -- has_export:'1'\n"
                           "┃     ├─ FunHeader -- type:'void'\n"
                           "┃     │  ├─ Var -- name:'__init'\n"
                           "┃     │  └─ NULL\n"
                           "┃     └─ FunBody\n"
                           "┃        ┢─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_dims0'\n"
                           "┃        ┃     └─ NULL\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_expr0'\n"
                           "┃        ┃     └─ NULL\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_var0'\n"
                           "┃        ┃     └─ NULL\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_dims1'\n"
                           "┃        ┃     └─ NULL\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_dims2'\n"
                           "┃        ┃     └─ NULL\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'float'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_expr2'\n"
                           "┃        ┃     └─ NULL\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_var2'\n"
                           "┃        ┃     └─ NULL\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_dims3'\n"
                           "┃        ┃     └─ NULL\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_dims4'\n"
                           "┃        ┃     └─ NULL\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'bool'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_expr4'\n"
                           "┃        ┃     └─ NULL\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_var4'\n"
                           "┃        ┃     └─ NULL\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_dims5'\n"
                           "┃        ┃     └─ NULL\n"
                           "┃        ┡─ NULL\n"
                           "┃        ├─ NULL\n"
                           "┃        ┢─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'@loop_dims0'\n"
                           "┃        ┃     └─ Int -- val:'2'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'da'\n"
                           "┃        ┃     └─ ProcCall\n"
                           "┃        ┃        ├─ Var -- name:'@alloc'\n"
                           "┃        ┃        ┢─ Exprs\n"
                           "┃        ┃        ┃  └─ Var -- name:'@loop_dims0'\n"
                           "┃        ┃        ┗─ NULL\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'@loop_expr0'\n"
                           "┃        ┃     └─ Int -- val:'4'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ForLoop\n"
                           "┃        ┃     ├─ Assign\n"
                           "┃        ┃     │  ├─ Var -- name:'@loop_var0'\n"
                           "┃        ┃     │  └─ Int -- val:'0'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_dims0'\n"
                           "┃        ┃     ├─ NULL\n"
                           "┃        ┃     ┢─ Statements\n"
                           "┃        ┃     ┃  └─ ArrayAssign\n"
                           "┃        ┃     ┃     ├─ ArrayExpr\n"
                           "┃        ┃     ┃     │  ┢─ Exprs\n"
                           "┃        ┃     ┃     │  ┃  └─ Var -- name:'@loop_var0'\n"
                           "┃        ┃     ┃     │  ┡─ NULL\n"
                           "┃        ┃     ┃     │  └─ Var -- name:'da'\n"
                           "┃        ┃     ┃     └─ Var -- name:'@loop_expr0'\n"
                           "┃        ┃     ┗─ NULL\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'@loop_dims1'\n"
                           "┃        ┃     └─ Int -- val:'3'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'db'\n"
                           "┃        ┃     └─ ProcCall\n"
                           "┃        ┃        ├─ Var -- name:'@alloc'\n"
                           "┃        ┃        ┢─ Exprs\n"
                           "┃        ┃        ┃  └─ Var -- name:'@loop_dims1'\n"
                           "┃        ┃        ┗─ NULL\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'@loop_dims2'\n"
                           "┃        ┃     └─ Int -- val:'4'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'ea'\n"
                           "┃        ┃     └─ ProcCall\n"
                           "┃        ┃        ├─ Var -- name:'@alloc'\n"
                           "┃        ┃        ┢─ Exprs\n"
                           "┃        ┃        ┃  └─ Var -- name:'@loop_dims2'\n"
                           "┃        ┃        ┗─ NULL\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'@loop_expr2'\n"
                           "┃        ┃     └─ Float -- val:'5.000000'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ForLoop\n"
                           "┃        ┃     ├─ Assign\n"
                           "┃        ┃     │  ├─ Var -- name:'@loop_var2'\n"
                           "┃        ┃     │  └─ Int -- val:'0'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_dims2'\n"
                           "┃        ┃     ├─ NULL\n"
                           "┃        ┃     ┢─ Statements\n"
                           "┃        ┃     ┃  └─ ArrayAssign\n"
                           "┃        ┃     ┃     ├─ ArrayExpr\n"
                           "┃        ┃     ┃     │  ┢─ Exprs\n"
                           "┃        ┃     ┃     │  ┃  └─ Var -- name:'@loop_var2'\n"
                           "┃        ┃     ┃     │  ┡─ NULL\n"
                           "┃        ┃     ┃     │  └─ Var -- name:'ea'\n"
                           "┃        ┃     ┃     └─ Var -- name:'@loop_expr2'\n"
                           "┃        ┃     ┗─ NULL\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'@loop_dims3'\n"
                           "┃        ┃     └─ Int -- val:'6'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'eb'\n"
                           "┃        ┃     └─ ProcCall\n"
                           "┃        ┃        ├─ Var -- name:'@alloc'\n"
                           "┃        ┃        ┢─ Exprs\n"
                           "┃        ┃        ┃  └─ Var -- name:'@loop_dims3'\n"
                           "┃        ┃        ┗─ NULL\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'@loop_dims4'\n"
                           "┃        ┃     └─ Int -- val:'7'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'fa'\n"
                           "┃        ┃     └─ ProcCall\n"
                           "┃        ┃        ├─ Var -- name:'@alloc'\n"
                           "┃        ┃        ┢─ Exprs\n"
                           "┃        ┃        ┃  └─ Var -- name:'@loop_dims4'\n"
                           "┃        ┃        ┗─ NULL\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'@loop_expr4'\n"
                           "┃        ┃     └─ Bool -- val:'1'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ForLoop\n"
                           "┃        ┃     ├─ Assign\n"
                           "┃        ┃     │  ├─ Var -- name:'@loop_var4'\n"
                           "┃        ┃     │  └─ Int -- val:'0'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_dims4'\n"
                           "┃        ┃     ├─ NULL\n"
                           "┃        ┃     ┢─ Statements\n"
                           "┃        ┃     ┃  └─ ArrayAssign\n"
                           "┃        ┃     ┃     ├─ ArrayExpr\n"
                           "┃        ┃     ┃     │  ┢─ Exprs\n"
                           "┃        ┃     ┃     │  ┃  └─ Var -- name:'@loop_var4'\n"
                           "┃        ┃     ┃     │  ┡─ NULL\n"
                           "┃        ┃     ┃     │  └─ Var -- name:'fa'\n"
                           "┃        ┃     ┃     └─ Var -- name:'@loop_expr4'\n"
                           "┃        ┃     ┗─ NULL\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'@loop_dims5'\n"
                           "┃        ┃     └─ Int -- val:'9'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'fb'\n"
                           "┃        ┃     └─ ProcCall\n"
                           "┃        ┃        ├─ Var -- name:'@alloc'\n"
                           "┃        ┃        ┢─ Exprs\n"
                           "┃        ┃        ┃  └─ Var -- name:'@loop_dims5'\n"
                           "┃        ┃        ┗─ NULL\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'aa'\n"
                           "┃        ┃     └─ Int -- val:'2'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'ba'\n"
                           "┃        ┃     └─ Float -- val:'3.000000'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'ca'\n"
                           "┃        ┃     └─ Bool -- val:'1'\n"
                           "┃        ┗─ NULL\n"
                           "┗─ NULL\n";

    ASSERT_MLSTREQ(expected, root_string);

    expected = "┌─ 0: Program\n"
               "├─ __init: FunHeader -- type:'void' -- Params: (null)\n"
               "├─ aa: VarDec -- type:'int'\n"
               "├─ ab: VarDec -- type:'int'\n"
               "├─ ba: VarDec -- type:'float'\n"
               "├─ bb: VarDec -- type:'float'\n"
               "├─ ca: VarDec -- type:'bool'\n"
               "├─ cb: VarDec -- type:'bool'\n"
               "├─ da: VarDec -- type:'int'\n"
               "├─ db: VarDec -- type:'int'\n"
               "├─ ea: VarDec -- type:'float'\n"
               "├─ eb: VarDec -- type:'float'\n"
               "├─ fa: VarDec -- type:'bool'\n"
               "├─ fb: VarDec -- type:'bool'\n"
               "└────────────────────\n"
               "\n"
               "┌─ 1: FunDef '__init' -- parent: '0: Program'\n"
               "├─ @loop_dims0: VarDec -- type:'int'\n"
               "├─ @loop_dims1: VarDec -- type:'int'\n"
               "├─ @loop_dims2: VarDec -- type:'int'\n"
               "├─ @loop_dims3: VarDec -- type:'int'\n"
               "├─ @loop_dims4: VarDec -- type:'int'\n"
               "├─ @loop_dims5: VarDec -- type:'int'\n"
               "├─ @loop_expr0: VarDec -- type:'int'\n"
               "├─ @loop_expr2: VarDec -- type:'float'\n"
               "├─ @loop_expr4: VarDec -- type:'bool'\n"
               "├─ @loop_var0: VarDec -- type:'int'\n"
               "├─ @loop_var2: VarDec -- type:'int'\n"
               "├─ @loop_var4: VarDec -- type:'int'\n"
               "└────────────────────\n";

    ASSERT_MLSTREQ(expected, symbols_string);
}

TEST_F(CodeGenPrepTest, ArrayFun)
{
    SetUp("milestone_6/array_fun/main.cvc");
    ASSERT_NE(nullptr, root);

    const char *expected = "Program\n"
                           "┢─ Declarations\n"
                           "┃  └─ GlobalDef -- has_export:'0'\n"
                           "┃     └─ VarDec -- type:'int'\n"
                           "┃        ├─ Var -- name:'k'\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ FunDef -- has_export:'0'\n"
                           "┃     ├─ FunHeader -- type:'void'\n"
                           "┃     │  ├─ Var -- name:'@fun_fun'\n"
                           "┃     │  ┢─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ Var -- name:'k'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ ArrayVar\n"
                           "┃     │  ┃     ┢─ DimensionVars\n"
                           "┃     │  ┃     ┃  └─ Var -- name:'k'\n"
                           "┃     │  ┃     ┡─ NULL\n"
                           "┃     │  ┃     └─ Var -- name:'arr'\n"
                           "┃     │  ┗─ NULL\n"
                           "┃     └─ FunBody\n"
                           "┃        ┢─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_dims0'\n"
                           "┃        ┃     └─ Int -- val:'4'\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_expr0'\n"
                           "┃        ┃     └─ NULL\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_var0'\n"
                           "┃        ┃     └─ NULL\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Int -- val:'4'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr2'\n"
                           "┃        ┃     └─ NULL\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'m'\n"
                           "┃        ┃     └─ Int -- val:'5'\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'n'\n"
                           "┃        ┃     └─ Int -- val:'6'\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'@temp_0'\n"
                           "┃        ┃     └─ Binop -- op:'+'\n"
                           "┃        ┃        ├─ Var -- name:'m'\n"
                           "┃        ┃        └─ Var -- name:'n'\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_dims1'\n"
                           "┃        ┃     └─ Binop -- op:'*'\n"
                           "┃        ┃        ├─ Var -- name:'@temp_0'\n"
                           "┃        ┃        └─ Var -- name:'n'\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Var -- name:'@temp_0'\n"
                           "┃        ┃     │  ┣─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Var -- name:'n'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr3'\n"
                           "┃        ┃     └─ NULL\n"
                           "┃        ┡─ NULL\n"
                           "┃        ├─ NULL\n"
                           "┃        ┢─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'arr2'\n"
                           "┃        ┃     └─ ProcCall\n"
                           "┃        ┃        ├─ Var -- name:'@alloc'\n"
                           "┃        ┃        ┢─ Exprs\n"
                           "┃        ┃        ┃  └─ Var -- name:'@loop_dims0'\n"
                           "┃        ┃        ┗─ NULL\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'@loop_expr0'\n"
                           "┃        ┃     └─ Int -- val:'2'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ForLoop\n"
                           "┃        ┃     ├─ Assign\n"
                           "┃        ┃     │  ├─ Var -- name:'@loop_var0'\n"
                           "┃        ┃     │  └─ Int -- val:'0'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_dims0'\n"
                           "┃        ┃     ├─ NULL\n"
                           "┃        ┃     ┢─ Statements\n"
                           "┃        ┃     ┃  └─ ArrayAssign\n"
                           "┃        ┃     ┃     ├─ ArrayExpr\n"
                           "┃        ┃     ┃     │  ┢─ Exprs\n"
                           "┃        ┃     ┃     │  ┃  └─ Var -- name:'@loop_var0'\n"
                           "┃        ┃     ┃     │  ┡─ NULL\n"
                           "┃        ┃     ┃     │  └─ Var -- name:'arr2'\n"
                           "┃        ┃     ┃     └─ Var -- name:'@loop_expr0'\n"
                           "┃        ┃     ┗─ NULL\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'arr3'\n"
                           "┃        ┃     └─ ProcCall\n"
                           "┃        ┃        ├─ Var -- name:'@alloc'\n"
                           "┃        ┃        ┢─ Exprs\n"
                           "┃        ┃        ┃  └─ Var -- name:'@loop_dims1'\n"
                           "┃        ┃        ┗─ NULL\n"
                           "┃        ┗─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ FunDef -- has_export:'0'\n"
                           "┃     ├─ FunHeader -- type:'void'\n"
                           "┃     │  ├─ Var -- name:'@fun_fukk'\n"
                           "┃     │  ┢─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ Var -- name:'k'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ ArrayVar\n"
                           "┃     │  ┃     ┢─ DimensionVars\n"
                           "┃     │  ┃     ┃  └─ Var -- name:'k'\n"
                           "┃     │  ┃     ┡─ NULL\n"
                           "┃     │  ┃     └─ Var -- name:'arr'\n"
                           "┃     │  ┗─ NULL\n"
                           "┃     └─ FunBody\n"
                           "┃        ┢─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'@temp_0'\n"
                           "┃        ┃     └─ Binop -- op:'+'\n"
                           "┃        ┃        ├─ Int -- val:'3'\n"
                           "┃        ┃        └─ Int -- val:'3'\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'@temp_1'\n"
                           "┃        ┃     └─ Binop -- op:'+'\n"
                           "┃        ┃        ├─ Int -- val:'2'\n"
                           "┃        ┃        └─ Int -- val:'2'\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_dims0'\n"
                           "┃        ┃     └─ Binop -- op:'*'\n"
                           "┃        ┃        ├─ Var -- name:'@temp_0'\n"
                           "┃        ┃        └─ Var -- name:'@temp_1'\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Var -- name:'@temp_0'\n"
                           "┃        ┃     │  ┣─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Var -- name:'@temp_1'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'arr3'\n"
                           "┃        ┃     └─ NULL\n"
                           "┃        ┡─ NULL\n"
                           "┃        ├─ NULL\n"
                           "┃        ┢─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'arr3'\n"
                           "┃        ┃     └─ ProcCall\n"
                           "┃        ┃        ├─ Var -- name:'@alloc'\n"
                           "┃        ┃        ┢─ Exprs\n"
                           "┃        ┃        ┃  └─ Var -- name:'@loop_dims0'\n"
                           "┃        ┃        ┗─ NULL\n"
                           "┃        ┗─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ FunDef -- has_export:'1'\n"
                           "┃     ├─ FunHeader -- type:'void'\n"
                           "┃     │  ├─ Var -- name:'__init'\n"
                           "┃     │  └─ NULL\n"
                           "┃     └─ FunBody\n"
                           "┃        ├─ NULL\n"
                           "┃        ├─ NULL\n"
                           "┃        ┢─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'k'\n"
                           "┃        ┃     └─ Int -- val:'7'\n"
                           "┃        ┗─ NULL\n"
                           "┗─ NULL\n";

    ASSERT_MLSTREQ(expected, root_string);

    expected =
        "┌─ 0: Program\n"
        "├─ k: VarDec -- type:'int'\n"
        "├─ __init: FunHeader -- type:'void' -- Params: (null)\n"
        "├─ @fun_fukk: FunHeader -- type:'void' -- Params: int (Var -- name:'k'), int (ArrayVar)\n"
        "├─ @fun_fun: FunHeader -- type:'void' -- Params: int (Var -- name:'k'), int (ArrayVar)\n"
        "└────────────────────\n"
        "\n"
        "┌─ 1: FunDef '@fun_fun' -- parent: '0: Program'\n"
        "├─ k: DimensionVars\n"
        "├─ m: VarDec -- type:'int'\n"
        "├─ n: VarDec -- type:'int'\n"
        "├─ @loop_dims0: VarDec -- type:'int'\n"
        "├─ arr2: VarDec -- type:'int'\n"
        "├─ @loop_dims1: VarDec -- type:'int'\n"
        "├─ arr3: VarDec -- type:'int'\n"
        "├─ arr: Params -- type:'int'\n"
        "├─ @loop_expr0: VarDec -- type:'int'\n"
        "├─ @temp_0: VarDec -- type:'int'\n"
        "├─ @loop_var0: VarDec -- type:'int'\n"
        "└────────────────────\n"
        "\n"
        "┌─ 1: FunDef '@fun_fukk' -- parent: '0: Program'\n"
        "├─ k: DimensionVars\n"
        "├─ @loop_dims0: VarDec -- type:'int'\n"
        "├─ arr3: VarDec -- type:'int'\n"
        "├─ arr: Params -- type:'int'\n"
        "├─ @temp_0: VarDec -- type:'int'\n"
        "├─ @temp_1: VarDec -- type:'int'\n"
        "└────────────────────\n"
        "\n"
        "┌─ 1: FunDef '__init' -- parent: '0: Program'\n"
        "└────────────────────\n";

    ASSERT_MLSTREQ(expected, symbols_string);
}

TEST_F(CodeGenPrepTest, ArrayParams)
{
    SetUp("milestone_8/arr_params/main.cvc");
    ASSERT_NE(nullptr, root);

    const char *expected = "Program\n"
                           "┢─ Declarations\n"
                           "┃  └─ FunDef -- has_export:'0'\n"
                           "┃     ├─ FunHeader -- type:'void'\n"
                           "┃     │  ├─ Var -- name:'@fun_fun'\n"
                           "┃     │  ┢─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ Var -- name:'n'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ Var -- name:'m'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ ArrayVar\n"
                           "┃     │  ┃     ┢─ DimensionVars\n"
                           "┃     │  ┃     ┃  └─ Var -- name:'m'\n"
                           "┃     │  ┃     ┣─ DimensionVars\n"
                           "┃     │  ┃     ┃  └─ Var -- name:'n'\n"
                           "┃     │  ┃     ┡─ NULL\n"
                           "┃     │  ┃     └─ Var -- name:'arr'\n"
                           "┃     │  ┗─ NULL\n"
                           "┃     └─ FunBody\n"
                           "┃        ┢─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'a'\n"
                           "┃        ┃     └─ Binop -- op:'+'\n"
                           "┃        ┃        ├─ Int -- val:'1'\n"
                           "┃        ┃        └─ Int -- val:'2'\n"
                           "┃        ┡─ NULL\n"
                           "┃        ├─ NULL\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ FunDef -- has_export:'1'\n"
                           "┃     ├─ FunHeader -- type:'void'\n"
                           "┃     │  ├─ Var -- name:'__init'\n"
                           "┃     │  └─ NULL\n"
                           "┃     └─ FunBody\n"
                           "┃        ├─ NULL\n"
                           "┃        ├─ NULL\n"
                           "┃        └─ NULL\n"
                           "┗─ NULL\n";

    ASSERT_MLSTREQ(expected, root_string);

    expected = "┌─ 0: Program\n"
               "├─ __init: FunHeader -- type:'void' -- Params: (null)\n"
               "├─ @fun_fun: FunHeader -- type:'void' -- Params: int (Var -- name:'n'), int (Var "
               "-- name:'m'), int (ArrayVar)\n"
               "└────────────────────\n"
               "\n"
               "┌─ 1: FunDef '@fun_fun' -- parent: '0: Program'\n"
               "├─ a: VarDec -- type:'int'\n"
               "├─ m: DimensionVars\n"
               "├─ n: DimensionVars\n"
               "├─ arr: Params -- type:'int'\n"
               "└────────────────────\n"
               "\n"
               "┌─ 1: FunDef '__init' -- parent: '0: Program'\n"
               "└────────────────────\n";

    ASSERT_MLSTREQ(expected, symbols_string);
}

TEST_F(CodeGenPrepTest, ShortCircuit)
{
    SetUp("milestone_9/main.cvc");
    ASSERT_NE(nullptr, root);

    const char *expected = "Program\n"
                           "┢─ Declarations\n"
                           "┃  └─ GlobalDef -- has_export:'0'\n"
                           "┃     └─ VarDec -- type:'bool'\n"
                           "┃        ├─ Var -- name:'a1'\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ GlobalDef -- has_export:'0'\n"
                           "┃     └─ VarDec -- type:'bool'\n"
                           "┃        ├─ Var -- name:'a2'\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ GlobalDef -- has_export:'0'\n"
                           "┃     └─ VarDec -- type:'bool'\n"
                           "┃        ├─ Var -- name:'a3'\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ GlobalDef -- has_export:'0'\n"
                           "┃     └─ VarDec -- type:'bool'\n"
                           "┃        ├─ Var -- name:'a'\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ FunDef -- has_export:'0'\n"
                           "┃     ├─ FunHeader -- type:'void'\n"
                           "┃     │  ├─ Var -- name:'@fun_test'\n"
                           "┃     │  └─ NULL\n"
                           "┃     └─ FunBody\n"
                           "┃        ┢─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'bool'\n"
                           "┃        ┃     ├─ Var -- name:'b'\n"
                           "┃        ┃     └─ Bool -- val:'0'\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'c'\n"
                           "┃        ┃     └─ Binop -- op:'+'\n"
                           "┃        ┃        ├─ Int -- val:'1'\n"
                           "┃        ┃        └─ Binop -- op:'*'\n"
                           "┃        ┃           ├─ Int -- val:'2'\n"
                           "┃        ┃           └─ Ternary\n"
                           "┃        ┃              ├─ Ternary\n"
                           "┃        ┃              │  ├─ Var -- name:'a'\n"
                           "┃        ┃              │  ├─ Bool -- val:'1'\n"
                           "┃        ┃              │  └─ Binop -- op:'+'\n"
                           "┃        ┃              │     ├─ Var -- name:'b'\n"
                           "┃        ┃              │     └─ Binop -- op:'*'\n"
                           "┃        ┃              │        ├─ Var -- name:'b'\n"
                           "┃        ┃              │        └─ Var -- name:'a'\n"
                           "┃        ┃              ├─ Int -- val:'1'\n"
                           "┃        ┃              └─ Int -- val:'0'\n"
                           "┃        ┡─ NULL\n"
                           "┃        ├─ NULL\n"
                           "┃        ┢─ Statements\n"
                           "┃        ┃  └─ IfStatement\n"
                           "┃        ┃     ├─ Ternary\n"
                           "┃        ┃     │  ├─ Var -- name:'a'\n"
                           "┃        ┃     │  ├─ Ternary\n"
                           "┃        ┃     │  │  ├─ Var -- name:'b'\n"
                           "┃        ┃     │  │  ├─ Bool -- val:'1'\n"
                           "┃        ┃     │  │  └─ Monop -- op:'!'\n"
                           "┃        ┃     │  │     └─ Var -- name:'a'\n"
                           "┃        ┃     │  └─ Bool -- val:'0'\n"
                           "┃        ┃     ┢─ Statements\n"
                           "┃        ┃     ┃  └─ RetStatement\n"
                           "┃        ┃     ┃     └─ NULL\n"
                           "┃        ┃     ┡─ NULL\n"
                           "┃        ┃     └─ NULL\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ WhileLoop\n"
                           "┃        ┃     ├─ Ternary\n"
                           "┃        ┃     │  ├─ Ternary\n"
                           "┃        ┃     │  │  ├─ Ternary\n"
                           "┃        ┃     │  │  │  ├─ Ternary\n"
                           "┃        ┃     │  │  │  │  ├─ Ternary\n"
                           "┃        ┃     │  │  │  │  │  ├─ Ternary\n"
                           "┃        ┃     │  │  │  │  │  │  ├─ Var -- name:'a'\n"
                           "┃        ┃     │  │  │  │  │  │  ├─ Var -- name:'a'\n"
                           "┃        ┃     │  │  │  │  │  │  └─ Bool -- val:'0'\n"
                           "┃        ┃     │  │  │  │  │  ├─ Var -- name:'a'\n"
                           "┃        ┃     │  │  │  │  │  └─ Bool -- val:'0'\n"
                           "┃        ┃     │  │  │  │  ├─ Var -- name:'a'\n"
                           "┃        ┃     │  │  │  │  └─ Bool -- val:'0'\n"
                           "┃        ┃     │  │  │  ├─ Bool -- val:'1'\n"
                           "┃        ┃     │  │  │  └─ Var -- name:'b'\n"
                           "┃        ┃     │  │  ├─ Bool -- val:'1'\n"
                           "┃        ┃     │  │  └─ Var -- name:'b'\n"
                           "┃        ┃     │  ├─ Bool -- val:'1'\n"
                           "┃        ┃     │  └─ Ternary\n"
                           "┃        ┃     │     ├─ Var -- name:'b'\n"
                           "┃        ┃     │     ├─ Var -- name:'a'\n"
                           "┃        ┃     │     └─ Bool -- val:'0'\n"
                           "┃        ┃     ┢─ Statements\n"
                           "┃        ┃     ┃  └─ RetStatement\n"
                           "┃        ┃     ┃     └─ NULL\n"
                           "┃        ┃     ┗─ NULL\n"
                           "┃        ┗─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ FunDef -- has_export:'1'\n"
                           "┃     ├─ FunHeader -- type:'void'\n"
                           "┃     │  ├─ Var -- name:'__init'\n"
                           "┃     │  └─ NULL\n"
                           "┃     └─ FunBody\n"
                           "┃        ├─ NULL\n"
                           "┃        ├─ NULL\n"
                           "┃        ┢─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'a'\n"
                           "┃        ┃     └─ Ternary\n"
                           "┃        ┃        ├─ Ternary\n"
                           "┃        ┃        │  ├─ Var -- name:'a1'\n"
                           "┃        ┃        │  ├─ Var -- name:'a2'\n"
                           "┃        ┃        │  └─ Bool -- val:'0'\n"
                           "┃        ┃        ├─ Var -- name:'a3'\n"
                           "┃        ┃        └─ Bool -- val:'0'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'a3'\n"
                           "┃        ┃     └─ Bool -- val:'1'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'a2'\n"
                           "┃        ┃     └─ Bool -- val:'0'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'a1'\n"
                           "┃        ┃     └─ Bool -- val:'1'\n"
                           "┃        ┗─ NULL\n"
                           "┗─ NULL\n";

    ASSERT_MLSTREQ(expected, root_string);

    expected = "┌─ 0: Program\n"
               "├─ a: VarDec -- type:'bool'\n"
               "├─ __init: FunHeader -- type:'void' -- Params: (null)\n"
               "├─ @fun_test: FunHeader -- type:'void' -- Params: (null)\n"
               "├─ a1: VarDec -- type:'bool'\n"
               "├─ a2: VarDec -- type:'bool'\n"
               "├─ a3: VarDec -- type:'bool'\n"
               "└────────────────────\n"
               "\n"
               "┌─ 1: FunDef '@fun_test' -- parent: '0: Program'\n"
               "├─ b: VarDec -- type:'bool'\n"
               "├─ c: VarDec -- type:'int'\n"
               "└────────────────────\n"
               "\n"
               "┌─ 1: FunDef '__init' -- parent: '0: Program'\n"
               "└────────────────────\n";

    ASSERT_MLSTREQ(expected, symbols_string);
}

TEST_F(CodeGenPrepTest, BooleanCast)
{
    SetUp("milestone_10/main.cvc");
    ASSERT_NE(nullptr, root);

    const char *expected = "Program\n"
                           "┢─ Declarations\n"
                           "┃  └─ FunDef -- has_export:'0'\n"
                           "┃     ├─ FunHeader -- type:'void'\n"
                           "┃     │  ├─ Var -- name:'@fun_test'\n"
                           "┃     │  └─ NULL\n"
                           "┃     └─ FunBody\n"
                           "┃        ┢─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'a'\n"
                           "┃        ┃     └─ Ternary\n"
                           "┃        ┃        ├─ Bool -- val:'0'\n"
                           "┃        ┃        ├─ Int -- val:'1'\n"
                           "┃        ┃        └─ Int -- val:'0'\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'b'\n"
                           "┃        ┃     └─ Cast -- type:'int'\n"
                           "┃        ┃        └─ Float -- val:'0.000000'\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'c'\n"
                           "┃        ┃     └─ Int -- val:'0'\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'float'\n"
                           "┃        ┃     ├─ Var -- name:'d'\n"
                           "┃        ┃     └─ Ternary\n"
                           "┃        ┃        ├─ Bool -- val:'1'\n"
                           "┃        ┃        ├─ Float -- val:'1.000000'\n"
                           "┃        ┃        └─ Float -- val:'0.000000'\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'float'\n"
                           "┃        ┃     ├─ Var -- name:'e'\n"
                           "┃        ┃     └─ Float -- val:'3.000000'\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'float'\n"
                           "┃        ┃     ├─ Var -- name:'f'\n"
                           "┃        ┃     └─ Cast -- type:'float'\n"
                           "┃        ┃        └─ Int -- val:'3'\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'bool'\n"
                           "┃        ┃     ├─ Var -- name:'g'\n"
                           "┃        ┃     └─ Bool -- val:'0'\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'bool'\n"
                           "┃        ┃     ├─ Var -- name:'h'\n"
                           "┃        ┃     └─ Ternary\n"
                           "┃        ┃        ├─ Binop -- op:'!='\n"
                           "┃        ┃        │  ├─ Float -- val:'10.000000'\n"
                           "┃        ┃        │  └─ Float -- val:'0.000000'\n"
                           "┃        ┃        ├─ Bool -- val:'1'\n"
                           "┃        ┃        └─ Bool -- val:'0'\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'bool'\n"
                           "┃        ┃     ├─ Var -- name:'j'\n"
                           "┃        ┃     └─ Ternary\n"
                           "┃        ┃        ├─ Binop -- op:'!='\n"
                           "┃        ┃        │  ├─ Int -- val:'10'\n"
                           "┃        ┃        │  └─ Int -- val:'0'\n"
                           "┃        ┃        ├─ Bool -- val:'1'\n"
                           "┃        ┃        └─ Bool -- val:'0'\n"
                           "┃        ┡─ NULL\n"
                           "┃        ├─ NULL\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ FunDef -- has_export:'1'\n"
                           "┃     ├─ FunHeader -- type:'void'\n"
                           "┃     │  ├─ Var -- name:'__init'\n"
                           "┃     │  └─ NULL\n"
                           "┃     └─ FunBody\n"
                           "┃        ├─ NULL\n"
                           "┃        ├─ NULL\n"
                           "┃        └─ NULL\n"
                           "┗─ NULL\n";

    ASSERT_MLSTREQ(expected, root_string);

    expected = "┌─ 0: Program\n"
               "├─ __init: FunHeader -- type:'void' -- Params: (null)\n"
               "├─ @fun_test: FunHeader -- type:'void' -- Params: (null)\n"
               "└────────────────────\n"
               "\n"
               "┌─ 1: FunDef '@fun_test' -- parent: '0: Program'\n"
               "├─ a: VarDec -- type:'int'\n"
               "├─ b: VarDec -- type:'int'\n"
               "├─ c: VarDec -- type:'int'\n"
               "├─ d: VarDec -- type:'float'\n"
               "├─ e: VarDec -- type:'float'\n"
               "├─ f: VarDec -- type:'float'\n"
               "├─ g: VarDec -- type:'bool'\n"
               "├─ h: VarDec -- type:'bool'\n"
               "├─ j: VarDec -- type:'bool'\n"
               "└────────────────────\n"
               "\n"
               "┌─ 1: FunDef '__init' -- parent: '0: Program'\n"
               "└────────────────────\n";

    ASSERT_MLSTREQ(expected, symbols_string);
}

TEST_F(CodeGenPrepTest, ArrayProccall)
{
    SetUp("milestone_8/arr_proccall/main.cvc");
    ASSERT_NE(nullptr, root);

    const char *expected = "Program\n"
                           "┢─ Declarations\n"
                           "┃  └─ FunDef -- has_export:'0'\n"
                           "┃     ├─ FunHeader -- type:'void'\n"
                           "┃     │  ├─ Var -- name:'@fun_fun_one'\n"
                           "┃     │  ┢─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ Var -- name:'b'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ Var -- name:'a'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ ArrayVar\n"
                           "┃     │  ┃     ┢─ DimensionVars\n"
                           "┃     │  ┃     ┃  └─ Var -- name:'a'\n"
                           "┃     │  ┃     ┣─ DimensionVars\n"
                           "┃     │  ┃     ┃  └─ Var -- name:'b'\n"
                           "┃     │  ┃     ┡─ NULL\n"
                           "┃     │  ┃     └─ Var -- name:'e'\n"
                           "┃     │  ┗─ NULL\n"
                           "┃     └─ FunBody\n"
                           "┃        ├─ NULL\n"
                           "┃        ├─ NULL\n"
                           "┃        ┢─ Statements\n"
                           "┃        ┃  └─ ProcCall\n"
                           "┃        ┃     ├─ Var -- name:'@fun_fun_two'\n"
                           "┃        ┃     ┢─ Exprs\n"
                           "┃        ┃     ┃  └─ Var -- name:'b'\n"
                           "┃        ┃     ┣─ Exprs\n"
                           "┃        ┃     ┃  └─ Var -- name:'a'\n"
                           "┃        ┃     ┣─ Exprs\n"
                           "┃        ┃     ┃  └─ Var -- name:'e'\n"
                           "┃        ┃     ┗─ NULL\n"
                           "┃        ┗─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ FunDef -- has_export:'0'\n"
                           "┃     ├─ FunHeader -- type:'void'\n"
                           "┃     │  ├─ Var -- name:'@fun_fun_two'\n"
                           "┃     │  ┢─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ Var -- name:'d'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ Var -- name:'c'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ ArrayVar\n"
                           "┃     │  ┃     ┢─ DimensionVars\n"
                           "┃     │  ┃     ┃  └─ Var -- name:'c'\n"
                           "┃     │  ┃     ┣─ DimensionVars\n"
                           "┃     │  ┃     ┃  └─ Var -- name:'d'\n"
                           "┃     │  ┃     ┡─ NULL\n"
                           "┃     │  ┃     └─ Var -- name:'f'\n"
                           "┃     │  ┗─ NULL\n"
                           "┃     └─ FunBody\n"
                           "┃        ┢─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'g'\n"
                           "┃        ┃     └─ Int -- val:'2'\n"
                           "┃        ┡─ NULL\n"
                           "┃        ├─ NULL\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ FunDef -- has_export:'1'\n"
                           "┃     ├─ FunHeader -- type:'void'\n"
                           "┃     │  ├─ Var -- name:'__init'\n"
                           "┃     │  └─ NULL\n"
                           "┃     └─ FunBody\n"
                           "┃        ├─ NULL\n"
                           "┃        ├─ NULL\n"
                           "┃        └─ NULL\n"
                           "┗─ NULL\n";

    ASSERT_MLSTREQ(expected, root_string);

    expected = "┌─ 0: Program\n"
               "├─ __init: FunHeader -- type:'void' -- Params: (null)\n"
               "├─ @fun_fun_one: FunHeader -- type:'void' -- Params: int (Var -- name:'b'), int "
               "(Var -- name:'a'), int (ArrayVar)\n"
               "├─ @fun_fun_two: FunHeader -- type:'void' -- Params: int (Var -- name:'d'), int "
               "(Var -- name:'c'), int (ArrayVar)\n"
               "└────────────────────\n"
               "\n"
               "┌─ 1: FunDef '@fun_fun_one' -- parent: '0: Program'\n"
               "├─ a: DimensionVars\n"
               "├─ b: DimensionVars\n"
               "├─ e: Params -- type:'int'\n"
               "└────────────────────\n"
               "\n"
               "┌─ 1: FunDef '@fun_fun_two' -- parent: '0: Program'\n"
               "├─ c: DimensionVars\n"
               "├─ d: DimensionVars\n"
               "├─ f: Params -- type:'int'\n"
               "├─ g: VarDec -- type:'int'\n"
               "└────────────────────\n"
               "\n"
               "┌─ 1: FunDef '__init' -- parent: '0: Program'\n"
               "└────────────────────\n";

    ASSERT_MLSTREQ(expected, symbols_string);
}

TEST_F(CodeGenPrepTest, DimensionReduction)
{
    SetUp("milestone_11/main.cvc");
    ASSERT_NE(nullptr, root);

    const char *expected =
        "Program\n"
        "┢─ Declarations\n"
        "┃  └─ GlobalDef -- has_export:'0'\n"
        "┃     └─ VarDec -- type:'int'\n"
        "┃        ├─ Var -- name:'m'\n"
        "┃        └─ NULL\n"
        "┣─ Declarations\n"
        "┃  └─ GlobalDef -- has_export:'0'\n"
        "┃     └─ VarDec -- type:'int'\n"
        "┃        ├─ Var -- name:'n'\n"
        "┃        └─ NULL\n"
        "┣─ Declarations\n"
        "┃  └─ GlobalDef -- has_export:'0'\n"
        "┃     └─ VarDec -- type:'int'\n"
        "┃        ├─ ArrayExpr\n"
        "┃        │  ┢─ Exprs\n"
        "┃        │  ┃  └─ Var -- name:'m'\n"
        "┃        │  ┣─ Exprs\n"
        "┃        │  ┃  └─ Var -- name:'n'\n"
        "┃        │  ┡─ NULL\n"
        "┃        │  └─ Var -- name:'a'\n"
        "┃        └─ NULL\n"
        "┣─ Declarations\n"
        "┃  └─ GlobalDec -- type:'int'\n"
        "┃     └─ ArrayVar\n"
        "┃        ┢─ DimensionVars\n"
        "┃        ┃  └─ Var -- name:'m1'\n"
        "┃        ┣─ DimensionVars\n"
        "┃        ┃  └─ Var -- name:'n2'\n"
        "┃        ┡─ NULL\n"
        "┃        └─ Var -- name:'b'\n"
        "┣─ Declarations\n"
        "┃  └─ FunDef -- has_export:'0'\n"
        "┃     ├─ FunHeader -- type:'void'\n"
        "┃     │  ├─ Var -- name:'@fun_test'\n"
        "┃     │  ┢─ Params -- type:'int'\n"
        "┃     │  ┃  └─ Var -- name:'r'\n"
        "┃     │  ┣─ Params -- type:'int'\n"
        "┃     │  ┃  └─ Var -- name:'n'\n"
        "┃     │  ┣─ Params -- type:'int'\n"
        "┃     │  ┃  └─ Var -- name:'l'\n"
        "┃     │  ┣─ Params -- type:'int'\n"
        "┃     │  ┃  └─ Var -- name:'k'\n"
        "┃     │  ┣─ Params -- type:'int'\n"
        "┃     │  ┃  └─ ArrayVar\n"
        "┃     │  ┃     ┢─ DimensionVars\n"
        "┃     │  ┃     ┃  └─ Var -- name:'k'\n"
        "┃     │  ┃     ┣─ DimensionVars\n"
        "┃     │  ┃     ┃  └─ Var -- name:'l'\n"
        "┃     │  ┃     ┣─ DimensionVars\n"
        "┃     │  ┃     ┃  └─ Var -- name:'n'\n"
        "┃     │  ┃     ┣─ DimensionVars\n"
        "┃     │  ┃     ┃  └─ Var -- name:'r'\n"
        "┃     │  ┃     ┡─ NULL\n"
        "┃     │  ┃     └─ Var -- name:'c'\n"
        "┃     │  ┗─ NULL\n"
        "┃     └─ FunBody\n"
        "┃        ┢─ VarDecs\n"
        "┃        ┃  └─ VarDec -- type:'int'\n"
        "┃        ┃     ├─ Var -- name:'@loop_expr0'\n"
        "┃        ┃     └─ NULL\n"
        "┃        ┣─ VarDecs\n"
        "┃        ┃  └─ VarDec -- type:'int'\n"
        "┃        ┃     ├─ Var -- name:'@loop_var0'\n"
        "┃        ┃     └─ NULL\n"
        "┃        ┣─ VarDecs\n"
        "┃        ┃  └─ VarDec -- type:'int'\n"
        "┃        ┃     ├─ Var -- name:'@loop_dims0'\n"
        "┃        ┃     └─ Binop -- op:'*'\n"
        "┃        ┃        ├─ Binop -- op:'*'\n"
        "┃        ┃        │  ├─ Binop -- op:'*'\n"
        "┃        ┃        │  │  ├─ Var -- name:'k'\n"
        "┃        ┃        │  │  └─ Var -- name:'l'\n"
        "┃        ┃        │  └─ Var -- name:'n'\n"
        "┃        ┃        └─ Var -- name:'r'\n"
        "┃        ┣─ VarDecs\n"
        "┃        ┃  └─ VarDec -- type:'int'\n"
        "┃        ┃     ├─ ArrayExpr\n"
        "┃        ┃     │  ┢─ Exprs\n"
        "┃        ┃     │  ┃  └─ Var -- name:'k'\n"
        "┃        ┃     │  ┣─ Exprs\n"
        "┃        ┃     │  ┃  └─ Var -- name:'l'\n"
        "┃        ┃     │  ┣─ Exprs\n"
        "┃        ┃     │  ┃  └─ Var -- name:'n'\n"
        "┃        ┃     │  ┣─ Exprs\n"
        "┃        ┃     │  ┃  └─ Var -- name:'r'\n"
        "┃        ┃     │  ┡─ NULL\n"
        "┃        ┃     │  └─ Var -- name:'d'\n"
        "┃        ┃     └─ NULL\n"
        "┃        ┣─ VarDecs\n"
        "┃        ┃  └─ VarDec -- type:'int'\n"
        "┃        ┃     ├─ Var -- name:'@for0_ik'\n"
        "┃        ┃     └─ NULL\n"
        "┃        ┣─ VarDecs\n"
        "┃        ┃  └─ VarDec -- type:'int'\n"
        "┃        ┃     ├─ Var -- name:'@for1_il'\n"
        "┃        ┃     └─ NULL\n"
        "┃        ┣─ VarDecs\n"
        "┃        ┃  └─ VarDec -- type:'int'\n"
        "┃        ┃     ├─ Var -- name:'@for2_in'\n"
        "┃        ┃     └─ NULL\n"
        "┃        ┣─ VarDecs\n"
        "┃        ┃  └─ VarDec -- type:'int'\n"
        "┃        ┃     ├─ Var -- name:'@for3_ir'\n"
        "┃        ┃     └─ NULL\n"
        "┃        ┣─ VarDecs\n"
        "┃        ┃  └─ VarDec -- type:'int'\n"
        "┃        ┃     ├─ Var -- name:'@temp_0'\n"
        "┃        ┃     └─ NULL\n"
        "┃        ┣─ VarDecs\n"
        "┃        ┃  └─ VarDec -- type:'int'\n"
        "┃        ┃     ├─ Var -- name:'@temp_1'\n"
        "┃        ┃     └─ NULL\n"
        "┃        ┣─ VarDecs\n"
        "┃        ┃  └─ VarDec -- type:'int'\n"
        "┃        ┃     ├─ Var -- name:'@temp_2'\n"
        "┃        ┃     └─ NULL\n"
        "┃        ┣─ VarDecs\n"
        "┃        ┃  └─ VarDec -- type:'int'\n"
        "┃        ┃     ├─ Var -- name:'@temp_3'\n"
        "┃        ┃     └─ NULL\n"
        "┃        ┡─ NULL\n"
        "┃        ├─ NULL\n"
        "┃        ┢─ Statements\n"
        "┃        ┃  └─ Assign\n"
        "┃        ┃     ├─ Var -- name:'@temp_0'\n"
        "┃        ┃     └─ Var -- name:'k'\n"
        "┃        ┣─ Statements\n"
        "┃        ┃  └─ Assign\n"
        "┃        ┃     ├─ Var -- name:'d'\n"
        "┃        ┃     └─ ProcCall\n"
        "┃        ┃        ├─ Var -- name:'@alloc'\n"
        "┃        ┃        ┢─ Exprs\n"
        "┃        ┃        ┃  └─ Var -- name:'@loop_dims0'\n"
        "┃        ┃        ┗─ NULL\n"
        "┃        ┣─ Statements\n"
        "┃        ┃  └─ Assign\n"
        "┃        ┃     ├─ Var -- name:'@loop_expr0'\n"
        "┃        ┃     └─ Int -- val:'5'\n"
        "┃        ┣─ Statements\n"
        "┃        ┃  └─ ForLoop\n"
        "┃        ┃     ├─ Assign\n"
        "┃        ┃     │  ├─ Var -- name:'@loop_var0'\n"
        "┃        ┃     │  └─ Int -- val:'0'\n"
        "┃        ┃     ├─ Var -- name:'@loop_dims0'\n"
        "┃        ┃     ├─ NULL\n"
        "┃        ┃     ┢─ Statements\n"
        "┃        ┃     ┃  └─ ArrayAssign\n"
        "┃        ┃     ┃     ├─ ArrayExpr\n"
        "┃        ┃     ┃     │  ┢─ Exprs\n"
        "┃        ┃     ┃     │  ┃  └─ Var -- name:'@loop_var0'\n"
        "┃        ┃     ┃     │  ┡─ NULL\n"
        "┃        ┃     ┃     │  └─ Var -- name:'d'\n"
        "┃        ┃     ┃     └─ Var -- name:'@loop_expr0'\n"
        "┃        ┃     ┗─ NULL\n"
        "┃        ┣─ Statements\n"
        "┃        ┃  └─ ForLoop\n"
        "┃        ┃     ├─ Assign\n"
        "┃        ┃     │  ├─ Var -- name:'@for0_ik'\n"
        "┃        ┃     │  └─ Int -- val:'0'\n"
        "┃        ┃     ├─ Var -- name:'@temp_0'\n"
        "┃        ┃     ├─ NULL\n"
        "┃        ┃     ┢─ Statements\n"
        "┃        ┃     ┃  └─ Assign\n"
        "┃        ┃     ┃     ├─ Var -- name:'@temp_1'\n"
        "┃        ┃     ┃     └─ Var -- name:'l'\n"
        "┃        ┃     ┣─ Statements\n"
        "┃        ┃     ┃  └─ ForLoop\n"
        "┃        ┃     ┃     ├─ Assign\n"
        "┃        ┃     ┃     │  ├─ Var -- name:'@for1_il'\n"
        "┃        ┃     ┃     │  └─ Int -- val:'0'\n"
        "┃        ┃     ┃     ├─ Var -- name:'@temp_1'\n"
        "┃        ┃     ┃     ├─ NULL\n"
        "┃        ┃     ┃     ┢─ Statements\n"
        "┃        ┃     ┃     ┃  └─ Assign\n"
        "┃        ┃     ┃     ┃     ├─ Var -- name:'@temp_2'\n"
        "┃        ┃     ┃     ┃     └─ Var -- name:'n'\n"
        "┃        ┃     ┃     ┣─ Statements\n"
        "┃        ┃     ┃     ┃  └─ ForLoop\n"
        "┃        ┃     ┃     ┃     ├─ Assign\n"
        "┃        ┃     ┃     ┃     │  ├─ Var -- name:'@for2_in'\n"
        "┃        ┃     ┃     ┃     │  └─ Int -- val:'0'\n"
        "┃        ┃     ┃     ┃     ├─ Var -- name:'@temp_2'\n"
        "┃        ┃     ┃     ┃     ├─ NULL\n"
        "┃        ┃     ┃     ┃     ┢─ Statements\n"
        "┃        ┃     ┃     ┃     ┃  └─ Assign\n"
        "┃        ┃     ┃     ┃     ┃     ├─ Var -- name:'@temp_3'\n"
        "┃        ┃     ┃     ┃     ┃     └─ Var -- name:'r'\n"
        "┃        ┃     ┃     ┃     ┣─ Statements\n"
        "┃        ┃     ┃     ┃     ┃  └─ ForLoop\n"
        "┃        ┃     ┃     ┃     ┃     ├─ Assign\n"
        "┃        ┃     ┃     ┃     ┃     │  ├─ Var -- name:'@for3_ir'\n"
        "┃        ┃     ┃     ┃     ┃     │  └─ Int -- val:'0'\n"
        "┃        ┃     ┃     ┃     ┃     ├─ Var -- name:'@temp_3'\n"
        "┃        ┃     ┃     ┃     ┃     ├─ NULL\n"
        "┃        ┃     ┃     ┃     ┃     ┢─ Statements\n"
        "┃        ┃     ┃     ┃     ┃     ┃  └─ ArrayAssign\n"
        "┃        ┃     ┃     ┃     ┃     ┃     ├─ ArrayExpr\n"
        "┃        ┃     ┃     ┃     ┃     ┃     │  ┢─ Exprs\n"
        "┃        ┃     ┃     ┃     ┃     ┃     │  ┃  └─ Binop -- op:'+'\n"
        "┃        ┃     ┃     ┃     ┃     ┃     │  ┃     ├─ Binop -- op:'*'\n"
        "┃        ┃     ┃     ┃     ┃     ┃     │  ┃     │  ├─ Binop -- op:'+'\n"
        "┃        ┃     ┃     ┃     ┃     ┃     │  ┃     │  │  ├─ Binop -- op:'*'\n"
        "┃        ┃     ┃     ┃     ┃     ┃     │  ┃     │  │  │  ├─ Binop -- op:'+'\n"
        "┃        ┃     ┃     ┃     ┃     ┃     │  ┃     │  │  │  │  ├─ Binop -- op:'*'\n"
        "┃        ┃     ┃     ┃     ┃     ┃     │  ┃     │  │  │  │  │  ├─ Var -- name:'@for0_ik'\n"
        "┃        ┃     ┃     ┃     ┃     ┃     │  ┃     │  │  │  │  │  └─ Var -- name:'k'\n"
        "┃        ┃     ┃     ┃     ┃     ┃     │  ┃     │  │  │  │  └─ Var -- name:'@for1_il'\n"
        "┃        ┃     ┃     ┃     ┃     ┃     │  ┃     │  │  │  └─ Var -- name:'l'\n"
        "┃        ┃     ┃     ┃     ┃     ┃     │  ┃     │  │  └─ Var -- name:'@for2_in'\n"
        "┃        ┃     ┃     ┃     ┃     ┃     │  ┃     │  └─ Var -- name:'n'\n"
        "┃        ┃     ┃     ┃     ┃     ┃     │  ┃     └─ Var -- name:'@for3_ir'\n"
        "┃        ┃     ┃     ┃     ┃     ┃     │  ┡─ NULL\n"
        "┃        ┃     ┃     ┃     ┃     ┃     │  └─ Var -- name:'d'\n"
        "┃        ┃     ┃     ┃     ┃     ┃     └─ Binop -- op:'+'\n"
        "┃        ┃     ┃     ┃     ┃     ┃        ├─ ArrayExpr\n"
        "┃        ┃     ┃     ┃     ┃     ┃        │  ┢─ Exprs\n"
        "┃        ┃     ┃     ┃     ┃     ┃        │  ┃  └─ Binop -- op:'+'\n"
        "┃        ┃     ┃     ┃     ┃     ┃        │  ┃     ├─ Binop -- op:'*'\n"
        "┃        ┃     ┃     ┃     ┃     ┃        │  ┃     │  ├─ Binop -- op:'+'\n"
        "┃        ┃     ┃     ┃     ┃     ┃        │  ┃     │  │  ├─ Binop -- op:'*'\n"
        "┃        ┃     ┃     ┃     ┃     ┃        │  ┃     │  │  │  ├─ Binop -- op:'+'\n"
        "┃        ┃     ┃     ┃     ┃     ┃        │  ┃     │  │  │  │  ├─ Binop -- op:'*'\n"
        "┃        ┃     ┃     ┃     ┃     ┃        │  ┃     │  │  │  │  │  ├─ Var -- "
        "name:'@for0_ik'\n"
        "┃        ┃     ┃     ┃     ┃     ┃        │  ┃     │  │  │  │  │  └─ Var -- name:'k'\n"
        "┃        ┃     ┃     ┃     ┃     ┃        │  ┃     │  │  │  │  └─ Var -- name:'@for1_il'\n"
        "┃        ┃     ┃     ┃     ┃     ┃        │  ┃     │  │  │  └─ Var -- name:'l'\n"
        "┃        ┃     ┃     ┃     ┃     ┃        │  ┃     │  │  └─ Var -- name:'@for2_in'\n"
        "┃        ┃     ┃     ┃     ┃     ┃        │  ┃     │  └─ Var -- name:'n'\n"
        "┃        ┃     ┃     ┃     ┃     ┃        │  ┃     └─ Var -- name:'@for3_ir'\n"
        "┃        ┃     ┃     ┃     ┃     ┃        │  ┡─ NULL\n"
        "┃        ┃     ┃     ┃     ┃     ┃        │  └─ Var -- name:'c'\n"
        "┃        ┃     ┃     ┃     ┃     ┃        └─ Binop -- op:'*'\n"
        "┃        ┃     ┃     ┃     ┃     ┃           ├─ ArrayExpr\n"
        "┃        ┃     ┃     ┃     ┃     ┃           │  ┢─ Exprs\n"
        "┃        ┃     ┃     ┃     ┃     ┃           │  ┃  └─ Binop -- op:'+'\n"
        "┃        ┃     ┃     ┃     ┃     ┃           │  ┃     ├─ Binop -- op:'*'\n"
        "┃        ┃     ┃     ┃     ┃     ┃           │  ┃     │  ├─ Var -- name:'@for2_in'\n"
        "┃        ┃     ┃     ┃     ┃     ┃           │  ┃     │  └─ Var -- name:'m'\n"
        "┃        ┃     ┃     ┃     ┃     ┃           │  ┃     └─ Var -- name:'@for3_ir'\n"
        "┃        ┃     ┃     ┃     ┃     ┃           │  ┡─ NULL\n"
        "┃        ┃     ┃     ┃     ┃     ┃           │  └─ Var -- name:'a'\n"
        "┃        ┃     ┃     ┃     ┃     ┃           └─ ArrayExpr\n"
        "┃        ┃     ┃     ┃     ┃     ┃              ┢─ Exprs\n"
        "┃        ┃     ┃     ┃     ┃     ┃              ┃  └─ Binop -- op:'+'\n"
        "┃        ┃     ┃     ┃     ┃     ┃              ┃     ├─ Binop -- op:'*'\n"
        "┃        ┃     ┃     ┃     ┃     ┃              ┃     │  ├─ Var -- name:'@for2_in'\n"
        "┃        ┃     ┃     ┃     ┃     ┃              ┃     │  └─ Var -- name:'m1'\n"
        "┃        ┃     ┃     ┃     ┃     ┃              ┃     └─ Var -- name:'@for3_ir'\n"
        "┃        ┃     ┃     ┃     ┃     ┃              ┡─ NULL\n"
        "┃        ┃     ┃     ┃     ┃     ┃              └─ Var -- name:'b'\n"
        "┃        ┃     ┃     ┃     ┃     ┗─ NULL\n"
        "┃        ┃     ┃     ┃     ┗─ NULL\n"
        "┃        ┃     ┃     ┗─ NULL\n"
        "┃        ┃     ┗─ NULL\n"
        "┃        ┗─ NULL\n"
        "┣─ Declarations\n"
        "┃  └─ FunDef -- has_export:'1'\n"
        "┃     ├─ FunHeader -- type:'void'\n"
        "┃     │  ├─ Var -- name:'__init'\n"
        "┃     │  └─ NULL\n"
        "┃     └─ FunBody\n"
        "┃        ┢─ VarDecs\n"
        "┃        ┃  └─ VarDec -- type:'int'\n"
        "┃        ┃     ├─ Var -- name:'@loop_dims0'\n"
        "┃        ┃     └─ Binop -- op:'*'\n"
        "┃        ┃        ├─ Var -- name:'m'\n"
        "┃        ┃        └─ Var -- name:'n'\n"
        "┃        ┡─ NULL\n"
        "┃        ├─ NULL\n"
        "┃        ┢─ Statements\n"
        "┃        ┃  └─ Assign\n"
        "┃        ┃     ├─ Var -- name:'a'\n"
        "┃        ┃     └─ ProcCall\n"
        "┃        ┃        ├─ Var -- name:'@alloc'\n"
        "┃        ┃        ┢─ Exprs\n"
        "┃        ┃        ┃  └─ Var -- name:'@loop_dims0'\n"
        "┃        ┃        ┗─ NULL\n"
        "┃        ┗─ NULL\n"
        "┗─ NULL\n";

    ASSERT_MLSTREQ(expected, root_string);

    expected = "┌─ 0: Program\n"
               "├─ a: VarDec -- type:'int'\n"
               "├─ b: GlobalDec -- type:'int'\n"
               "├─ m: VarDec -- type:'int'\n"
               "├─ n: VarDec -- type:'int'\n"
               "├─ __init: FunHeader -- type:'void' -- Params: (null)\n"
               "├─ m1: DimensionVars\n"
               "├─ n2: DimensionVars\n"
               "├─ @fun_test: FunHeader -- type:'void' -- Params: int (Var -- name:'r'), int (Var "
               "-- name:'n'), int (Var -- name:'l'), int (Var -- name:'k'), int (ArrayVar)\n"
               "└────────────────────\n"
               "\n"
               "┌─ 1: FunDef '@fun_test' -- parent: '0: Program'\n"
               "├─ c: Params -- type:'int'\n"
               "├─ d: VarDec -- type:'int'\n"
               "├─ k: DimensionVars\n"
               "├─ l: DimensionVars\n"
               "├─ n: DimensionVars\n"
               "├─ r: DimensionVars\n"
               "├─ @loop_dims0: VarDec -- type:'int'\n"
               "├─ @for0_ik: VarDec -- type:'int'\n"
               "├─ @loop_expr0: VarDec -- type:'int'\n"
               "├─ @for1_il: VarDec -- type:'int'\n"
               "├─ @for2_in: VarDec -- type:'int'\n"
               "├─ @temp_0: VarDec -- type:'int'\n"
               "├─ @temp_1: VarDec -- type:'int'\n"
               "├─ @temp_2: VarDec -- type:'int'\n"
               "├─ @temp_3: VarDec -- type:'int'\n"
               "├─ @loop_var0: VarDec -- type:'int'\n"
               "├─ @for3_ir: VarDec -- type:'int'\n"
               "└────────────────────\n"
               "\n"
               "┌─ 1: FunDef '__init' -- parent: '0: Program'\n"
               "├─ @loop_dims0: VarDec -- type:'int'\n"
               "└────────────────────\n";

    ASSERT_MLSTREQ(expected, symbols_string);
}

TEST_F(CodeGenPrepTest, ArrTwoArrParams)
{
    SetUp("milestone_8/two_arr_params/main.cvc");
    ASSERT_NE(nullptr, root);

    const char *expected = "Program\n"
                           "┢─ Declarations\n"
                           "┃  └─ FunDef -- has_export:'0'\n"
                           "┃     ├─ FunHeader -- type:'void'\n"
                           "┃     │  ├─ Var -- name:'@fun_fun'\n"
                           "┃     │  ┢─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ Var -- name:'d'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ Var -- name:'c'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ Var -- name:'b'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ Var -- name:'a'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ ArrayVar\n"
                           "┃     │  ┃     ┢─ DimensionVars\n"
                           "┃     │  ┃     ┃  └─ Var -- name:'a'\n"
                           "┃     │  ┃     ┣─ DimensionVars\n"
                           "┃     │  ┃     ┃  └─ Var -- name:'b'\n"
                           "┃     │  ┃     ┡─ NULL\n"
                           "┃     │  ┃     └─ Var -- name:'arr1'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ ArrayVar\n"
                           "┃     │  ┃     ┢─ DimensionVars\n"
                           "┃     │  ┃     ┃  └─ Var -- name:'c'\n"
                           "┃     │  ┃     ┣─ DimensionVars\n"
                           "┃     │  ┃     ┃  └─ Var -- name:'d'\n"
                           "┃     │  ┃     ┡─ NULL\n"
                           "┃     │  ┃     └─ Var -- name:'arr2'\n"
                           "┃     │  ┗─ NULL\n"
                           "┃     └─ FunBody\n"
                           "┃        ┢─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'i'\n"
                           "┃        ┃     └─ Int -- val:'1'\n"
                           "┃        ┡─ NULL\n"
                           "┃        ├─ NULL\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ FunDef -- has_export:'1'\n"
                           "┃     ├─ FunHeader -- type:'void'\n"
                           "┃     │  ├─ Var -- name:'__init'\n"
                           "┃     │  └─ NULL\n"
                           "┃     └─ FunBody\n"
                           "┃        ├─ NULL\n"
                           "┃        ├─ NULL\n"
                           "┃        └─ NULL\n"
                           "┗─ NULL\n";

    ASSERT_MLSTREQ(expected, root_string);

    expected =
        "┌─ 0: Program\n"
        "├─ __init: FunHeader -- type:'void' -- Params: (null)\n"
        "├─ @fun_fun: FunHeader -- type:'void' -- Params: int (Var -- name:'d'), int (Var -- "
        "name:'c'), int (Var -- name:'b'), int (Var -- name:'a'), int (ArrayVar), int (ArrayVar)\n"
        "└────────────────────\n"
        "\n"
        "┌─ 1: FunDef '@fun_fun' -- parent: '0: Program'\n"
        "├─ a: DimensionVars\n"
        "├─ b: DimensionVars\n"
        "├─ c: DimensionVars\n"
        "├─ d: DimensionVars\n"
        "├─ i: VarDec -- type:'int'\n"
        "├─ arr1: Params -- type:'int'\n"
        "├─ arr2: Params -- type:'int'\n"
        "└────────────────────\n"
        "\n"
        "┌─ 1: FunDef '__init' -- parent: '0: Program'\n"
        "└────────────────────\n";

    ASSERT_MLSTREQ(expected, symbols_string);
}

TEST_F(CodeGenPrepTest, ArrTwoArrProccall)
{
    SetUp("milestone_8/two_arr_proccall/main.cvc");
    ASSERT_NE(nullptr, root);

    const char *expected = "Program\n"
                           "┢─ Declarations\n"
                           "┃  └─ FunDef -- has_export:'0'\n"
                           "┃     ├─ FunHeader -- type:'void'\n"
                           "┃     │  ├─ Var -- name:'@fun_fun_one'\n"
                           "┃     │  ┢─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ Var -- name:'d'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ Var -- name:'c'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ Var -- name:'b'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ Var -- name:'a'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ ArrayVar\n"
                           "┃     │  ┃     ┢─ DimensionVars\n"
                           "┃     │  ┃     ┃  └─ Var -- name:'a'\n"
                           "┃     │  ┃     ┣─ DimensionVars\n"
                           "┃     │  ┃     ┃  └─ Var -- name:'b'\n"
                           "┃     │  ┃     ┡─ NULL\n"
                           "┃     │  ┃     └─ Var -- name:'arr1'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ ArrayVar\n"
                           "┃     │  ┃     ┢─ DimensionVars\n"
                           "┃     │  ┃     ┃  └─ Var -- name:'c'\n"
                           "┃     │  ┃     ┣─ DimensionVars\n"
                           "┃     │  ┃     ┃  └─ Var -- name:'d'\n"
                           "┃     │  ┃     ┡─ NULL\n"
                           "┃     │  ┃     └─ Var -- name:'arr2'\n"
                           "┃     │  ┗─ NULL\n"
                           "┃     └─ FunBody\n"
                           "┃        ┢─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'i'\n"
                           "┃        ┃     └─ Int -- val:'1'\n"
                           "┃        ┡─ NULL\n"
                           "┃        ├─ NULL\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ FunDef -- has_export:'0'\n"
                           "┃     ├─ FunHeader -- type:'void'\n"
                           "┃     │  ├─ Var -- name:'@fun_fun_two'\n"
                           "┃     │  ┢─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ Var -- name:'h'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ Var -- name:'g'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ Var -- name:'f'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ Var -- name:'e'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ ArrayVar\n"
                           "┃     │  ┃     ┢─ DimensionVars\n"
                           "┃     │  ┃     ┃  └─ Var -- name:'e'\n"
                           "┃     │  ┃     ┣─ DimensionVars\n"
                           "┃     │  ┃     ┃  └─ Var -- name:'f'\n"
                           "┃     │  ┃     ┡─ NULL\n"
                           "┃     │  ┃     └─ Var -- name:'arr3'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ ArrayVar\n"
                           "┃     │  ┃     ┢─ DimensionVars\n"
                           "┃     │  ┃     ┃  └─ Var -- name:'g'\n"
                           "┃     │  ┃     ┣─ DimensionVars\n"
                           "┃     │  ┃     ┃  └─ Var -- name:'h'\n"
                           "┃     │  ┃     ┡─ NULL\n"
                           "┃     │  ┃     └─ Var -- name:'arr4'\n"
                           "┃     │  ┗─ NULL\n"
                           "┃     └─ FunBody\n"
                           "┃        ├─ NULL\n"
                           "┃        ├─ NULL\n"
                           "┃        ┢─ Statements\n"
                           "┃        ┃  └─ ProcCall\n"
                           "┃        ┃     ├─ Var -- name:'@fun_fun_one'\n"
                           "┃        ┃     ┢─ Exprs\n"
                           "┃        ┃     ┃  └─ Var -- name:'h'\n"
                           "┃        ┃     ┣─ Exprs\n"
                           "┃        ┃     ┃  └─ Var -- name:'g'\n"
                           "┃        ┃     ┣─ Exprs\n"
                           "┃        ┃     ┃  └─ Var -- name:'f'\n"
                           "┃        ┃     ┣─ Exprs\n"
                           "┃        ┃     ┃  └─ Var -- name:'e'\n"
                           "┃        ┃     ┣─ Exprs\n"
                           "┃        ┃     ┃  └─ Var -- name:'arr3'\n"
                           "┃        ┃     ┣─ Exprs\n"
                           "┃        ┃     ┃  └─ Var -- name:'arr4'\n"
                           "┃        ┃     ┗─ NULL\n"
                           "┃        ┗─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ FunDef -- has_export:'1'\n"
                           "┃     ├─ FunHeader -- type:'void'\n"
                           "┃     │  ├─ Var -- name:'__init'\n"
                           "┃     │  └─ NULL\n"
                           "┃     └─ FunBody\n"
                           "┃        ├─ NULL\n"
                           "┃        ├─ NULL\n"
                           "┃        └─ NULL\n"
                           "┗─ NULL\n";

    ASSERT_MLSTREQ(expected, root_string);

    expected =
        "┌─ 0: Program\n"
        "├─ __init: FunHeader -- type:'void' -- Params: (null)\n"
        "├─ @fun_fun_one: FunHeader -- type:'void' -- Params: int (Var -- name:'d'), int (Var -- "
        "name:'c'), int (Var -- name:'b'), int (Var -- name:'a'), int (ArrayVar), int (ArrayVar)\n"
        "├─ @fun_fun_two: FunHeader -- type:'void' -- Params: int (Var -- name:'h'), int (Var -- "
        "name:'g'), int (Var -- name:'f'), int (Var -- name:'e'), int (ArrayVar), int (ArrayVar)\n"
        "└────────────────────\n"
        "\n"
        "┌─ 1: FunDef '@fun_fun_one' -- parent: '0: Program'\n"
        "├─ a: DimensionVars\n"
        "├─ b: DimensionVars\n"
        "├─ c: DimensionVars\n"
        "├─ d: DimensionVars\n"
        "├─ i: VarDec -- type:'int'\n"
        "├─ arr1: Params -- type:'int'\n"
        "├─ arr2: Params -- type:'int'\n"
        "└────────────────────\n"
        "\n"
        "┌─ 1: FunDef '@fun_fun_two' -- parent: '0: Program'\n"
        "├─ e: DimensionVars\n"
        "├─ f: DimensionVars\n"
        "├─ g: DimensionVars\n"
        "├─ h: DimensionVars\n"
        "├─ arr3: Params -- type:'int'\n"
        "├─ arr4: Params -- type:'int'\n"
        "└────────────────────\n"
        "\n"
        "┌─ 1: FunDef '__init' -- parent: '0: Program'\n"
        "└────────────────────\n";

    ASSERT_MLSTREQ(expected, symbols_string);
}

TEST_F(CodeGenPrepTest, ArrDoubleArrProccall)
{
    SetUp("milestone_8/double_arr_proccall/main.cvc");
    ASSERT_NE(nullptr, root);

    const char *expected = "Program\n"
                           "┢─ Declarations\n"
                           "┃  └─ FunDef -- has_export:'0'\n"
                           "┃     ├─ FunHeader -- type:'void'\n"
                           "┃     │  ├─ Var -- name:'@fun_fun'\n"
                           "┃     │  ┢─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ Var -- name:'d'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ Var -- name:'c'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ Var -- name:'b'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ Var -- name:'a'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ ArrayVar\n"
                           "┃     │  ┃     ┢─ DimensionVars\n"
                           "┃     │  ┃     ┃  └─ Var -- name:'a'\n"
                           "┃     │  ┃     ┣─ DimensionVars\n"
                           "┃     │  ┃     ┃  └─ Var -- name:'b'\n"
                           "┃     │  ┃     ┡─ NULL\n"
                           "┃     │  ┃     └─ Var -- name:'arr1'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ ArrayVar\n"
                           "┃     │  ┃     ┢─ DimensionVars\n"
                           "┃     │  ┃     ┃  └─ Var -- name:'c'\n"
                           "┃     │  ┃     ┣─ DimensionVars\n"
                           "┃     │  ┃     ┃  └─ Var -- name:'d'\n"
                           "┃     │  ┃     ┡─ NULL\n"
                           "┃     │  ┃     └─ Var -- name:'arr2'\n"
                           "┃     │  ┗─ NULL\n"
                           "┃     └─ FunBody\n"
                           "┃        ├─ NULL\n"
                           "┃        ├─ NULL\n"
                           "┃        ┢─ Statements\n"
                           "┃        ┃  └─ ProcCall\n"
                           "┃        ┃     ├─ Var -- name:'@fun_fun2'\n"
                           "┃        ┃     ┢─ Exprs\n"
                           "┃        ┃     ┃  └─ Var -- name:'b'\n"
                           "┃        ┃     ┣─ Exprs\n"
                           "┃        ┃     ┃  └─ Var -- name:'a'\n"
                           "┃        ┃     ┣─ Exprs\n"
                           "┃        ┃     ┃  └─ Var -- name:'b'\n"
                           "┃        ┃     ┣─ Exprs\n"
                           "┃        ┃     ┃  └─ Var -- name:'a'\n"
                           "┃        ┃     ┣─ Exprs\n"
                           "┃        ┃     ┃  └─ Var -- name:'arr1'\n"
                           "┃        ┃     ┣─ Exprs\n"
                           "┃        ┃     ┃  └─ Var -- name:'arr1'\n"
                           "┃        ┃     ┗─ NULL\n"
                           "┃        ┗─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ FunDef -- has_export:'0'\n"
                           "┃     ├─ FunHeader -- type:'void'\n"
                           "┃     │  ├─ Var -- name:'@fun_fun2'\n"
                           "┃     │  ┢─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ Var -- name:'h'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ Var -- name:'g'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ Var -- name:'f'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ Var -- name:'e'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ ArrayVar\n"
                           "┃     │  ┃     ┢─ DimensionVars\n"
                           "┃     │  ┃     ┃  └─ Var -- name:'e'\n"
                           "┃     │  ┃     ┣─ DimensionVars\n"
                           "┃     │  ┃     ┃  └─ Var -- name:'f'\n"
                           "┃     │  ┃     ┡─ NULL\n"
                           "┃     │  ┃     └─ Var -- name:'arr3'\n"
                           "┃     │  ┣─ Params -- type:'int'\n"
                           "┃     │  ┃  └─ ArrayVar\n"
                           "┃     │  ┃     ┢─ DimensionVars\n"
                           "┃     │  ┃     ┃  └─ Var -- name:'g'\n"
                           "┃     │  ┃     ┣─ DimensionVars\n"
                           "┃     │  ┃     ┃  └─ Var -- name:'h'\n"
                           "┃     │  ┃     ┡─ NULL\n"
                           "┃     │  ┃     └─ Var -- name:'arr4'\n"
                           "┃     │  ┗─ NULL\n"
                           "┃     └─ FunBody\n"
                           "┃        ┢─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'i'\n"
                           "┃        ┃     └─ Int -- val:'1'\n"
                           "┃        ┡─ NULL\n"
                           "┃        ├─ NULL\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ FunDef -- has_export:'1'\n"
                           "┃     ├─ FunHeader -- type:'void'\n"
                           "┃     │  ├─ Var -- name:'__init'\n"
                           "┃     │  └─ NULL\n"
                           "┃     └─ FunBody\n"
                           "┃        ├─ NULL\n"
                           "┃        ├─ NULL\n"
                           "┃        └─ NULL\n"
                           "┗─ NULL\n";

    ASSERT_MLSTREQ(expected, root_string);

    expected =
        "┌─ 0: Program\n"
        "├─ __init: FunHeader -- type:'void' -- Params: (null)\n"
        "├─ @fun_fun2: FunHeader -- type:'void' -- Params: int (Var -- name:'h'), int (Var -- "
        "name:'g'), int (Var -- name:'f'), int (Var -- name:'e'), int (ArrayVar), int (ArrayVar)\n"
        "├─ @fun_fun: FunHeader -- type:'void' -- Params: int (Var -- name:'d'), int (Var -- "
        "name:'c'), int (Var -- name:'b'), int (Var -- name:'a'), int (ArrayVar), int (ArrayVar)\n"
        "└────────────────────\n"
        "\n"
        "┌─ 1: FunDef '@fun_fun' -- parent: '0: Program'\n"
        "├─ a: DimensionVars\n"
        "├─ b: DimensionVars\n"
        "├─ c: DimensionVars\n"
        "├─ d: DimensionVars\n"
        "├─ arr1: Params -- type:'int'\n"
        "├─ arr2: Params -- type:'int'\n"
        "└────────────────────\n"
        "\n"
        "┌─ 1: FunDef '@fun_fun2' -- parent: '0: Program'\n"
        "├─ e: DimensionVars\n"
        "├─ f: DimensionVars\n"
        "├─ g: DimensionVars\n"
        "├─ h: DimensionVars\n"
        "├─ i: VarDec -- type:'int'\n"
        "├─ arr3: Params -- type:'int'\n"
        "├─ arr4: Params -- type:'int'\n"
        "└────────────────────\n"
        "\n"
        "┌─ 1: FunDef '__init' -- parent: '0: Program'\n"
        "└────────────────────\n";

    ASSERT_MLSTREQ(expected, symbols_string);
}

TEST_F(CodeGenPrepTest, Functional_Assignment01_Array)
{
    SetUp("functional/assignment_01/array.cvc");
    ASSERT_NE(nullptr, root);

    // To long to check
    // ASSERT_MLSTREQ(expected, root_string);

    const char *expected =
        "┌─ 0: Program\n"
        "├─ @fun_scanFloat: FunHeader -- type:'float' -- Params: (null)\n"
        "├─ __init: FunHeader -- type:'void' -- Params: (null)\n"
        "├─ @fun_scanFloatVec: FunHeader -- type:'void' -- Params: int (Var -- name:'n'), float "
        "(ArrayVar)\n"
        "├─ @fun_printIntVec: FunHeader -- type:'void' -- Params: int (Var -- name:'n'), int "
        "(ArrayVar)\n"
        "├─ @fun_printFloatMat: FunHeader -- type:'void' -- Params: int (Var -- name:'n'), int "
        "(Var -- name:'m'), float (ArrayVar)\n"
        "├─ @fun_scanIntMat: FunHeader -- type:'void' -- Params: int (Var -- name:'n'), int (Var "
        "-- name:'m'), int (ArrayVar)\n"
        "├─ @fun_printFloatVec: FunHeader -- type:'void' -- Params: int (Var -- name:'n'), float "
        "(ArrayVar)\n"
        "├─ @fun_printInt: FunHeader -- type:'void' -- Params: int (Var -- name:'val')\n"
        "├─ @fun_scanInt: FunHeader -- type:'int' -- Params: (null)\n"
        "├─ @fun_scanIntVec: FunHeader -- type:'void' -- Params: int (Var -- name:'n'), int "
        "(ArrayVar)\n"
        "├─ @fun_matMul: FunHeader -- type:'bool' -- Params: int (Var -- name:'l'), int (Var -- "
        "name:'q'), int (Var -- name:'p'), int (Var -- name:'o'), int (Var -- name:'n'), int (Var "
        "-- name:'m'), float (ArrayVar), float (ArrayVar), float (ArrayVar)\n"
        "├─ @fun_printNewlines: FunHeader -- type:'void' -- Params: int (Var -- name:'num')\n"
        "├─ @fun_scanFloatMat: FunHeader -- type:'void' -- Params: int (Var -- name:'n'), int (Var "
        "-- name:'m'), float (ArrayVar)\n"
        "├─ @fun_queens: FunHeader -- type:'bool' -- Params: int (Var -- name:'n'), int (Var -- "
        "name:'m'), bool (ArrayVar)\n"
        "├─ @fun_printIntMat: FunHeader -- type:'void' -- Params: int (Var -- name:'n'), int (Var "
        "-- name:'m'), int (ArrayVar)\n"
        "├─ @fun_printSpaces: FunHeader -- type:'void' -- Params: int (Var -- name:'num')\n"
        "├─ @fun_printFloat: FunHeader -- type:'void' -- Params: float (Var -- name:'val')\n"
        "└────────────────────\n"
        "\n"
        "┌─ 1: FunDef '@fun_printIntVec' -- parent: '0: Program'\n"
        "├─ n: DimensionVars\n"
        "├─ vec: Params -- type:'int'\n"
        "├─ @temp_0: VarDec -- type:'int'\n"
        "├─ @for0_i: VarDec -- type:'int'\n"
        "└────────────────────\n"
        "\n"
        "┌─ 1: FunDef '@fun_printFloatVec' -- parent: '0: Program'\n"
        "├─ n: DimensionVars\n"
        "├─ vec: Params -- type:'float'\n"
        "├─ @temp_0: VarDec -- type:'int'\n"
        "├─ @for0_i: VarDec -- type:'int'\n"
        "└────────────────────\n"
        "\n"
        "┌─ 1: FunDef '@fun_printIntMat' -- parent: '0: Program'\n"
        "├─ @for1_j: VarDec -- type:'int'\n"
        "├─ m: DimensionVars\n"
        "├─ n: DimensionVars\n"
        "├─ mat: Params -- type:'int'\n"
        "├─ @temp_0: VarDec -- type:'int'\n"
        "├─ @temp_1: VarDec -- type:'int'\n"
        "├─ @for0_i: VarDec -- type:'int'\n"
        "└────────────────────\n"
        "\n"
        "┌─ 1: FunDef '@fun_printFloatMat' -- parent: '0: Program'\n"
        "├─ @for1_j: VarDec -- type:'int'\n"
        "├─ m: DimensionVars\n"
        "├─ n: DimensionVars\n"
        "├─ mat: Params -- type:'float'\n"
        "├─ @temp_0: VarDec -- type:'int'\n"
        "├─ @temp_1: VarDec -- type:'int'\n"
        "├─ @for0_i: VarDec -- type:'int'\n"
        "└────────────────────\n"
        "\n"
        "┌─ 1: FunDef '@fun_scanIntVec' -- parent: '0: Program'\n"
        "├─ n: DimensionVars\n"
        "├─ vec: Params -- type:'int'\n"
        "├─ @temp_0: VarDec -- type:'int'\n"
        "├─ @for0_i: VarDec -- type:'int'\n"
        "└────────────────────\n"
        "\n"
        "┌─ 1: FunDef '@fun_scanFloatVec' -- parent: '0: Program'\n"
        "├─ n: DimensionVars\n"
        "├─ vec: Params -- type:'float'\n"
        "├─ @temp_0: VarDec -- type:'int'\n"
        "├─ @for0_i: VarDec -- type:'int'\n"
        "└────────────────────\n"
        "\n"
        "┌─ 1: FunDef '@fun_scanIntMat' -- parent: '0: Program'\n"
        "├─ @for1_j: VarDec -- type:'int'\n"
        "├─ m: DimensionVars\n"
        "├─ n: DimensionVars\n"
        "├─ mat: Params -- type:'int'\n"
        "├─ @temp_0: VarDec -- type:'int'\n"
        "├─ @temp_1: VarDec -- type:'int'\n"
        "├─ @for0_i: VarDec -- type:'int'\n"
        "└────────────────────\n"
        "\n"
        "┌─ 1: FunDef '@fun_scanFloatMat' -- parent: '0: Program'\n"
        "├─ @for1_j: VarDec -- type:'int'\n"
        "├─ m: DimensionVars\n"
        "├─ n: DimensionVars\n"
        "├─ mat: Params -- type:'float'\n"
        "├─ @temp_0: VarDec -- type:'int'\n"
        "├─ @temp_1: VarDec -- type:'int'\n"
        "├─ @for0_i: VarDec -- type:'int'\n"
        "└────────────────────\n"
        "\n"
        "┌─ 1: FunDef '@fun_matMul' -- parent: '0: Program'\n"
        "├─ a: Params -- type:'float'\n"
        "├─ b: Params -- type:'float'\n"
        "├─ c: Params -- type:'float'\n"
        "├─ l: DimensionVars\n"
        "├─ m: DimensionVars\n"
        "├─ n: DimensionVars\n"
        "├─ o: DimensionVars\n"
        "├─ p: DimensionVars\n"
        "├─ q: DimensionVars\n"
        "├─ @for4_ik: VarDec -- type:'int'\n"
        "├─ @for0_im: VarDec -- type:'int'\n"
        "├─ @for1_ik: VarDec -- type:'int'\n"
        "├─ @for2_im: VarDec -- type:'int'\n"
        "├─ @temp_0: VarDec -- type:'int'\n"
        "├─ @temp_1: VarDec -- type:'int'\n"
        "├─ @temp_2: VarDec -- type:'int'\n"
        "├─ @temp_3: VarDec -- type:'int'\n"
        "├─ @temp_4: VarDec -- type:'int'\n"
        "├─ @for3_in: VarDec -- type:'int'\n"
        "└────────────────────\n"
        "\n"
        "┌─ 1: FunDef '@fun_queens' -- parent: '0: Program'\n"
        "├─ m: DimensionVars\n"
        "├─ n: DimensionVars\n"
        "├─ @loop_dims0: VarDec -- type:'int'\n"
        "├─ @loop_dims1: VarDec -- type:'int'\n"
        "├─ @loop_dims2: VarDec -- type:'int'\n"
        "├─ @fun_contains: FunHeader -- type:'bool' -- Params: int (Var -- name:'m'), int "
        "(ArrayVar), int (Var -- name:'value')\n"
        "├─ rows: VarDec -- type:'int'\n"
        "├─ @for0_im: VarDec -- type:'int'\n"
        "├─ @loop_expr0: VarDec -- type:'int'\n"
        "├─ @loop_expr1: VarDec -- type:'int'\n"
        "├─ @loop_expr2: VarDec -- type:'int'\n"
        "├─ diagnolNegative: VarDec -- type:'int'\n"
        "├─ startIndex: VarDec -- type:'int'\n"
        "├─ field: Params -- type:'bool'\n"
        "├─ @fun_solveQueens: FunHeader -- type:'bool' -- Params: int (Var -- name:'im')\n"
        "├─ @fun_isValid: FunHeader -- type:'bool' -- Params: (null)\n"
        "├─ @temp_0: VarDec -- type:'int'\n"
        "├─ @temp_1: VarDec -- type:'int'\n"
        "├─ @loop_var0: VarDec -- type:'int'\n"
        "├─ @loop_var1: VarDec -- type:'int'\n"
        "├─ @loop_var2: VarDec -- type:'int'\n"
        "├─ diagnolPositive: VarDec -- type:'int'\n"
        "└────────────────────\n"
        "\n"
        "┌─ 2: FunDef '@fun_contains' -- parent: '1: FunDef '@fun_queens''\n"
        "├─ m: DimensionVars\n"
        "├─ array: Params -- type:'int'\n"
        "├─ value: Params -- type:'int'\n"
        "├─ @temp_0: VarDec -- type:'int'\n"
        "├─ @for0_i: VarDec -- type:'int'\n"
        "└────────────────────\n"
        "\n"
        "┌─ 2: FunDef '@fun_isValid' -- parent: '1: FunDef '@fun_queens''\n"
        "├─ @for0_im: VarDec -- type:'int'\n"
        "├─ @for1_in: VarDec -- type:'int'\n"
        "├─ @temp_0: VarDec -- type:'int'\n"
        "├─ @temp_1: VarDec -- type:'int'\n"
        "└────────────────────\n"
        "\n"
        "┌─ 2: FunDef '@fun_solveQueens' -- parent: '1: FunDef '@fun_queens''\n"
        "├─ im: Params -- type:'int'\n"
        "├─ @for0_in: VarDec -- type:'int'\n"
        "├─ @temp_0: VarDec -- type:'int'\n"
        "└────────────────────\n"
        "\n"
        "┌─ 1: FunDef '__init' -- parent: '0: Program'\n"
        "└────────────────────\n";

    ASSERT_MLSTREQ(expected, symbols_string);
}

TEST_F(CodeGenPrepTest, EdgeCasesValid)
{
    SetUp("preperation/edgecase/valid.cvc");
    ASSERT_NE(nullptr, root);

    const char *expected = "Program\n"
                           "┢─ Declarations\n"
                           "┃  └─ GlobalDef -- has_export:'0'\n"
                           "┃     └─ VarDec -- type:'int'\n"
                           "┃        ├─ Var -- name:'@temp_1'\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ GlobalDef -- has_export:'0'\n"
                           "┃     └─ VarDec -- type:'int'\n"
                           "┃        ├─ Var -- name:'@temp_0'\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ GlobalDec -- type:'float'\n"
                           "┃     └─ Var -- name:'m'\n"
                           "┣─ Declarations\n"
                           "┃  └─ GlobalDef -- has_export:'1'\n"
                           "┃     └─ VarDec -- type:'int'\n"
                           "┃        ├─ Var -- name:'nine'\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ GlobalDec -- type:'float'\n"
                           "┃     └─ Var -- name:'e'\n"
                           "┣─ Declarations\n"
                           "┃  └─ GlobalDec -- type:'int'\n"
                           "┃     └─ Var -- name:'t'\n"
                           "┣─ Declarations\n"
                           "┃  └─ FunDef -- has_export:'1'\n"
                           "┃     ├─ FunHeader -- type:'void'\n"
                           "┃     │  ├─ Var -- name:'@fun_e'\n"
                           "┃     │  └─ NULL\n"
                           "┃     └─ FunBody\n"
                           "┃        ┢─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_dims0'\n"
                           "┃        ┃     └─ Var -- name:'nine'\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'bool'\n"
                           "┃        ┃     ├─ ArrayExpr\n"
                           "┃        ┃     │  ┢─ Exprs\n"
                           "┃        ┃     │  ┃  └─ Var -- name:'nine'\n"
                           "┃        ┃     │  ┡─ NULL\n"
                           "┃        ┃     │  └─ Var -- name:'nine'\n"
                           "┃        ┃     └─ NULL\n"
                           "┃        ┡─ NULL\n"
                           "┃        ├─ NULL\n"
                           "┃        ┢─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'nine'\n"
                           "┃        ┃     └─ ProcCall\n"
                           "┃        ┃        ├─ Var -- name:'@alloc'\n"
                           "┃        ┃        ┢─ Exprs\n"
                           "┃        ┃        ┃  └─ Var -- name:'@loop_dims0'\n"
                           "┃        ┃        ┗─ NULL\n"
                           "┃        ┗─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ GlobalDef -- has_export:'1'\n"
                           "┃     └─ VarDec -- type:'float'\n"
                           "┃        ├─ ArrayExpr\n"
                           "┃        │  ┢─ Exprs\n"
                           "┃        │  ┃  └─ Var -- name:'@temp_0'\n"
                           "┃        │  ┡─ NULL\n"
                           "┃        │  └─ Var -- name:'p'\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ FunDef -- has_export:'0'\n"
                           "┃     ├─ FunHeader -- type:'void'\n"
                           "┃     │  ├─ Var -- name:'@fun_m'\n"
                           "┃     │  └─ NULL\n"
                           "┃     └─ FunBody\n"
                           "┃        ├─ NULL\n"
                           "┃        ├─ NULL\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ GlobalDef -- has_export:'1'\n"
                           "┃     └─ VarDec -- type:'int'\n"
                           "┃        ├─ ArrayExpr\n"
                           "┃        │  ┢─ Exprs\n"
                           "┃        │  ┃  └─ Var -- name:'@temp_1'\n"
                           "┃        │  ┡─ NULL\n"
                           "┃        │  └─ Var -- name:'f'\n"
                           "┃        └─ NULL\n"
                           "┣─ Declarations\n"
                           "┃  └─ GlobalDec -- type:'float'\n"
                           "┃     └─ Var -- name:'c'\n"
                           "┣─ Declarations\n"
                           "┃  └─ FunDef -- has_export:'1'\n"
                           "┃     ├─ FunHeader -- type:'void'\n"
                           "┃     │  ├─ Var -- name:'__init'\n"
                           "┃     │  └─ NULL\n"
                           "┃     └─ FunBody\n"
                           "┃        ┢─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_expr1'\n"
                           "┃        ┃     └─ NULL\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_var1'\n"
                           "┃        ┃     └─ NULL\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'float'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_expr0'\n"
                           "┃        ┃     └─ NULL\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_var0'\n"
                           "┃        ┃     └─ NULL\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_dims0'\n"
                           "┃        ┃     └─ Var -- name:'@temp_0'\n"
                           "┃        ┣─ VarDecs\n"
                           "┃        ┃  └─ VarDec -- type:'int'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_dims1'\n"
                           "┃        ┃     └─ Var -- name:'@temp_1'\n"
                           "┃        ┡─ NULL\n"
                           "┃        ├─ NULL\n"
                           "┃        ┢─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'@temp_1'\n"
                           "┃        ┃     └─ Int -- val:'0'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'@temp_0'\n"
                           "┃        ┃     └─ Int -- val:'0'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'f'\n"
                           "┃        ┃     └─ ProcCall\n"
                           "┃        ┃        ├─ Var -- name:'@alloc'\n"
                           "┃        ┃        ┢─ Exprs\n"
                           "┃        ┃        ┃  └─ Var -- name:'@loop_dims1'\n"
                           "┃        ┃        ┗─ NULL\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'@loop_expr1'\n"
                           "┃        ┃     └─ Int -- val:'0'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ForLoop\n"
                           "┃        ┃     ├─ Assign\n"
                           "┃        ┃     │  ├─ Var -- name:'@loop_var1'\n"
                           "┃        ┃     │  └─ Int -- val:'0'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_dims1'\n"
                           "┃        ┃     ├─ NULL\n"
                           "┃        ┃     ┢─ Statements\n"
                           "┃        ┃     ┃  └─ ArrayAssign\n"
                           "┃        ┃     ┃     ├─ ArrayExpr\n"
                           "┃        ┃     ┃     │  ┢─ Exprs\n"
                           "┃        ┃     ┃     │  ┃  └─ Var -- name:'@loop_var1'\n"
                           "┃        ┃     ┃     │  ┡─ NULL\n"
                           "┃        ┃     ┃     │  └─ Var -- name:'f'\n"
                           "┃        ┃     ┃     └─ Var -- name:'@loop_expr1'\n"
                           "┃        ┃     ┗─ NULL\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'p'\n"
                           "┃        ┃     └─ ProcCall\n"
                           "┃        ┃        ├─ Var -- name:'@alloc'\n"
                           "┃        ┃        ┢─ Exprs\n"
                           "┃        ┃        ┃  └─ Var -- name:'@loop_dims0'\n"
                           "┃        ┃        ┗─ NULL\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ Assign\n"
                           "┃        ┃     ├─ Var -- name:'@loop_expr0'\n"
                           "┃        ┃     └─ Float -- val:'0.000000'\n"
                           "┃        ┣─ Statements\n"
                           "┃        ┃  └─ ForLoop\n"
                           "┃        ┃     ├─ Assign\n"
                           "┃        ┃     │  ├─ Var -- name:'@loop_var0'\n"
                           "┃        ┃     │  └─ Int -- val:'0'\n"
                           "┃        ┃     ├─ Var -- name:'@loop_dims0'\n"
                           "┃        ┃     ├─ NULL\n"
                           "┃        ┃     ┢─ Statements\n"
                           "┃        ┃     ┃  └─ ArrayAssign\n"
                           "┃        ┃     ┃     ├─ ArrayExpr\n"
                           "┃        ┃     ┃     │  ┢─ Exprs\n"
                           "┃        ┃     ┃     │  ┃  └─ Var -- name:'@loop_var0'\n"
                           "┃        ┃     ┃     │  ┡─ NULL\n"
                           "┃        ┃     ┃     │  └─ Var -- name:'p'\n"
                           "┃        ┃     ┃     └─ Var -- name:'@loop_expr0'\n"
                           "┃        ┃     ┗─ NULL\n"
                           "┃        ┗─ NULL\n"
                           "┗─ NULL\n";

    ASSERT_MLSTREQ(expected, root_string);

    expected = "┌─ 0: Program\n"
               "├─ c: GlobalDec -- type:'float'\n"
               "├─ e: GlobalDec -- type:'float'\n"
               "├─ f: VarDec -- type:'int'\n"
               "├─ m: GlobalDec -- type:'float'\n"
               "├─ p: VarDec -- type:'float'\n"
               "├─ __init: FunHeader -- type:'void' -- Params: (null)\n"
               "├─ t: GlobalDec -- type:'int'\n"
               "├─ nine: VarDec -- type:'int'\n"
               "├─ @fun_e: FunHeader -- type:'void' -- Params: (null)\n"
               "├─ @fun_m: FunHeader -- type:'void' -- Params: (null)\n"
               "├─ @temp_0: VarDec -- type:'int'\n"
               "├─ @temp_1: VarDec -- type:'int'\n"
               "└────────────────────\n"
               "\n"
               "┌─ 1: FunDef '@fun_e' -- parent: '0: Program'\n"
               "├─ @loop_dims0: VarDec -- type:'int'\n"
               "├─ nine: VarDec -- type:'bool'\n"
               "└────────────────────\n"
               "\n"
               "┌─ 1: FunDef '@fun_m' -- parent: '0: Program'\n"
               "└────────────────────\n"
               "\n"
               "┌─ 1: FunDef '__init' -- parent: '0: Program'\n"
               "├─ @loop_dims0: VarDec -- type:'int'\n"
               "├─ @loop_dims1: VarDec -- type:'int'\n"
               "├─ @loop_expr0: VarDec -- type:'float'\n"
               "├─ @loop_expr1: VarDec -- type:'int'\n"
               "├─ @loop_var0: VarDec -- type:'int'\n"
               "├─ @loop_var1: VarDec -- type:'int'\n"
               "└────────────────────\n";

    ASSERT_MLSTREQ(expected, symbols_string);
}

// /////////////////////////
// COMPILATION FAILURE tests
// /////////////////////////

TEST_F(CodeGenPrepTest, DoubleArrParams)
{
    SetUpNoExecute("milestone_8/double_arr_params/main.cvc");
    ASSERT_EXIT(
        run_code_gen_preparation(input_filepath.c_str()), testing::ExitedWithCode(1),
        testing::AllOf(testing::HasSubstr("already defined"), testing::HasSubstr(" 3 Error")));
}

TEST_F(CodeGenPrepTest, EdgeCaseInvalid)
{
    SetUpNoExecute("preperation/edgecase/invalid.cvc");
    ASSERT_EXIT(run_code_gen_preparation(input_filepath.c_str()), testing::ExitedWithCode(1),
                testing::AllOf(testing::HasSubstr("'a' already defined at 1:19 - 1:28"),
                               testing::HasSubstr("'a' was not declared"),
                               testing::HasSubstr(" 2 Error")));
}
